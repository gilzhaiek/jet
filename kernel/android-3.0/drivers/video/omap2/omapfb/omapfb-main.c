...
/* Don't need to use late_initcall(). Just need to make sure
** that the omapfb_init() occurs after the display's init().
** To do so, we can have the linker link the display's init
** before the omapfb init since the ordering of module_init's
** are determined by the link order of the modules.
*/
