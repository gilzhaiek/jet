...
#define VSYSREG_TRACKING_DISABLE		0x80
#define VSYSREG_4P4			0x2D
#define VSYSREG_3P54		0x02
...
/* TWL6032 BATTERY TEMPERATURE CHANNEL */
#define TWL6032_GPADC_TEMP_CHNL	0x02
...
#define VOLT_INIT_MEASURE_SIZE  5
#define VOLT_INIT_SPIKE_RMV_CNT 3
#define VOLT_INIT_MV_MAX_DIFF   100
...
struct twl6030_bci_device_info {
	...
	/* End of charge condtion check */
#ifdef CONFIG_MACH_OMAP4_JET
	bool charge_full_flag;
	unsigned long charge_full_time;
#endif
};
...
static int adc_to_temperature(struct twl6030_bci_device_info *di, int code)
{
	int adc_code = 0;
	int temp = 0;
	int i;
	...
	temp = temp/10;/* in tenths of degree Celsius */

	return temp;
}

static int twl6032_get_battery_temperature(struct twl6030_bci_device_info *di)
{
	struct twl6030_gpadc_request req;
	int adc_code = 0;
	int temp = 0;
	...
	req.channels = TWL6032_GPADC_TEMP_CHNL;
	req.method = TWL6030_GPADC_SW2;
	req.active = 0;
	req.func_cb = NULL;
	req.type = TWL6030_GPADC_WAIT;
	...
	temp = adc_to_temperature(di, adc_code);
...
static void twl6030_start_usb_charger_sw(struct twl6030_bci_device_info *di)
{
	...
	dev_dbg(di->dev, "[%s:%u] USB input current limit %dmA\n",__FUNCTION__,__LINE__,di->charger_incurrentmA);
...
#ifdef CONFIG_MACH_OMAP4_JET//JET-459, Avoid negative current from TI ENdofCharge document,disable track and reenable after 1 second
			twl_i2c_write_u8(TWL6032_MODULE_CHARGER, VSYSREG_4P4|VSYSREG_TRACKING_DISABLE,
						CHARGERUSB_VSYSREG);
			di->charge_full_time = jiffies;
			di->charge_full_flag= true;
#endif
...
static irqreturn_t twl6032charger_fault_interrupt_hw(int irq, void *_di)
{
...
#ifndef CONFIG_MACH_OMAP4_JET//PMIC 6032 no need state below
	...
	//TODO: The current system don't care of POWER_SUPPLY_HEALTH_UNSPEC_FAILURE
	if(di->bat_health == POWER_SUPPLY_HEALTH_UNSPEC_FAILURE){
		di->bat_health =POWER_SUPPLY_HEALTH_GOOD;
		return IRQ_HANDLED;
	}
	if (charger_stop) {
		//TODO: Need to take care of PMIC, stop charging?
		if (!(stat1 & (VBUS_DET | VAC_DET)))
...
/**
 *	A) Calculate battery impedance Zs using temperature compensation as follows:
 *			Zs = 0.32 + ((4.2 - Vmeas)^2*0.1) + (((40 - Tmeas)^2)/5000)
 *	B) Calculate normalized voltage as follows:
 *			V(100) = Vmeas - (Zs * (Imeas+100))
 *
 *	Using lookup table temperature_offset_table[] for (((40 - Tmeas)^2)/5) term
 *
 */
#define TEMPERATURE_OFFSET_MAX  40	//degC
#define TEMPERATURE_OFFSET_MIN -20	//degC
static int temperature_offset_table[] = {
		  // Temp [idx]
		0, // 40C [0]
		0, // 39C [1]
		1, // 38C [2]
...
static int get_temperature_offset(int temp_C)
{
	int offset = 0;
	...
	if (temp_C > TEMPERATURE_OFFSET_MAX)
		offset = 0;
	else if (temp_C < TEMPERATURE_OFFSET_MIN)
		offset = temperature_offset_table[TEMPERATURE_OFFSET_MAX - TEMPERATURE_OFFSET_MIN];
	else
		offset = temperature_offset_table[TEMPERATURE_OFFSET_MAX - temp_C];

#ifdef DEBUG
	printk(KERN_DEBUG "%s:[#%d]: Temp_C: %dC, offset: %d, idx: %d \n",
			__func__,__LINE__,temp_C,offset,TEMPERATURE_OFFSET_MAX - temp_C);
#endif
...
/*
 * calculate normalized volatge based on above stated formula.
 */
#define BAT_COEF_A 		320		//(0.32 x 1000)
#define BAT_COEF_B 		100		//(0.10 x 1000)
#define BAT_VOLT_MAX	4200	//mV
#ifdef CONFIG_JET_SUN
#define OCV_TABLE_mA	100		//Jet Sunglass ocv_table using 100mA discharge "SunglassBatteryTest_02Sep14_05Sep14.xlsx"
#else //CONFIG_JET_SNOW
#define OCV_TABLE_mA	130		//Jet Snow ocv_table using 130mA discharge "SnowConstantPowerTest.xlsx"
#endif

static int twl6030battery_calc_norm_voltage(struct twl6030_bci_device_info *di, int volt_now, int current_now, int temperature)
{
	int bat_Zs = -1;	//battery source impedance
	int bat_Vd = -1;	//battery voltage difference
	int offset_T = 0;	//battery temperature offset
	int volt_norm = 0;

	//calculate (4.2V - Vmeas)^2
	bat_Vd = (BAT_VOLT_MAX - volt_now);
	bat_Vd *=bat_Vd;
	bat_Vd /=1000;

#ifdef CONFIG_JET_SUN
	offset_T = get_temperature_offset(temperature);
#endif

	bat_Zs = BAT_COEF_A + ((bat_Vd * BAT_COEF_B)/1000) + offset_T;

	if(current_now <0) //discharging
	{
		volt_norm  = volt_now - ((bat_Zs * (current_now + OCV_TABLE_mA))/1000);
	}
	else //charging
	{
		volt_norm  = volt_now - ((bat_Zs * (current_now - OCV_TABLE_mA))/1000);
...
static int __init parse_cmdline_param(const char *param_to_read, int def_value) {
	const char *bootcmd_str = NULL;
	char temp_str[8] = { 0 };
	int temp = def_value;
	int ret = -1;

	bootcmd_str = strstr(boot_command_line, param_to_read);
	if(bootcmd_str != NULL) {
		strncpy(temp_str,(const char *)&bootcmd_str[strlen(param_to_read)],4);
		ret = kstrtoint(temp_str,10,&temp);
	}
#ifdef DEBUG
	printk(KERN_DEBUG "%s:[#%d] bootcmd_str= %s, temp_str= %s, param_size=%d, ret=%d, temp=%d \n",
			__func__,__LINE__, bootcmd_str, temp_str, strlen(param_to_read), ret, temp);
#endif
	return temp;
}

/*
 * Print twl6030 Status (voltage limits, last turnoff)
 *
 */
static int twl6030print_status(void)
{
	int ret = 0;
	u8 last_turnoff = 0;
	u8 vsysmin_lo = 0;
	...
	ret |= twl_i2c_read_u8(TWL_MODULE_PM_MASTER, &last_turnoff, TWL6032_LAST_TURNOFF_STS);
	if (!ret)
		printk(KERN_DEBUG "%s: last_turnoff=0x%X\n",__func__,last_turnoff);

	ret |= twl_i2c_read_u8(TWL_MODULE_PM_MASTER, &vsysmin_lo, TWL6032_VSYSMIN_LO_THRESHOLD);
	if (!ret)
		printk(KERN_DEBUG "%s: vsysmin_lo=0x%X\n",__func__,vsysmin_lo);

	ret |= twl_i2c_read_u8(TWL_MODULE_PM_MASTER, &vsysmin_hi, TWL6032_VSYSMIN_HI_THRESHOLD);
	if (!ret)
		printk(KERN_DEBUG "%s: vsysmin_hi=0x%X\n",__func__,vsysmin_hi);

	ret |= twl_i2c_read_u8(TWL_MODULE_PM_MASTER, &vbatmin_hi, TWL6032_VBATMIN_HI_THRESHOLD);
	if (!ret)
		printk(KERN_DEBUG "%s: vbatmin_hi=0x%X\n",__func__,vbatmin_hi);
...
static void twl6030_current_avg(struct work_struct *work)
{
	...
	int volt_norm = 0;
	int vbat_now_avg = 0;
	int current_now=0, current_z1=0;
	int idx = 100;
	...
	/*
	* Following algorithm synchronizes voltage and current changes.
	* Instantaneous current updates every 250 ms (approx.)
	* Wait for next current transition and then take average voltage until another next transition or timeout at 1 sec.
	* For 'current_now' use same instantaneous current value when average voltage is calculated.
	*/
	current_now = twl6030battery_current(di);
	current_z1 = current_now;
	while((current_z1 == current_now) && idx>0)
	{
		msleep(10);
		current_now = twl6030battery_current(di);
		idx--;
	...
	current_z1 = current_now;
	vbat_now_avg += twl6030_get_gpadc_conversion(di, di->gpadc_vbat_chnl);
	idx=1;

	while(1) {
		msleep(10);
		current_now = twl6030battery_current(di);
		if((current_z1 == current_now) && idx<100) {
			vbat_now_avg += twl6030_get_gpadc_conversion(di, di->gpadc_vbat_chnl);
			idx++;
		} else {
			vbat_now = vbat_now_avg/idx;
#ifdef CONFIG_JET_SUN
			di->current_uA = (current_z1 - (current_z1 * 10/100)); //Subtract 10% as FG current reading is 10% higher then measured
#else
			di->current_uA = current_z1;
#endif
			break;
		}
	}
	...
	/* Generate battery removal event (HEALTH=Dead) when USB is connected and voltage drops below 1V */
	if((vbat_now < 1000) &&
			(di->bat_health != POWER_SUPPLY_HEALTH_DEAD) &&
			(di->charger_source== POWER_SUPPLY_TYPE_USB))
	{
...
#ifdef CONFIG_JET_SUN
		current_avg_uA -= (current_avg_uA * 10/100); //Subtract 10% as FG current reading is 10% higher then measured
#endif
		di->current_avg_uA = current_avg_uA * 1000;
	}

	volt_norm = twl6030battery_calc_norm_voltage(di, vbat_now, di->current_uA/1000, di->temp_C);

	di->voltage_mV = volt_norm;
	di->raw_voltage_mV = vbat_now;

#ifdef CONFIG_POWER_SUPPLY_DEBUG
	printk(KERN_DEBUG "%s:[#%d]: SOC=%d, CURRENT_now=%d, CURRENT_av=%d, VOLT_raw=%d, volt_norm=%d, temp_C=%d\n",
			__func__,__LINE__,di->cell.soc, di->current_uA/1000, di->current_avg_uA/1000, vbat_now, volt_norm,di->temp_C);
...
#ifdef CONFIG_FUEL_GAUGE
#ifdef LOCAL_DEBUG
static short filter_voltage(struct cell_state *cell, short voltage) {
	if(voltage > (cell->av_voltage*(100+MAX_VOLTAGE_CHANGE_PER)/100)) {
		return cell->av_voltage*(100+MAX_VOLTAGE_CHANGE_PER)/100;
	}
	else if(voltage < (cell->av_voltage*(100-MAX_VOLTAGE_CHANGE_PER)/100)) {
		return cell->av_voltage*(100-MAX_VOLTAGE_CHANGE_PER)/100;
	}
	return voltage;
}
...
static int capacity_changed(struct twl6030_bci_device_info *di)
{
	...
	if(delta_q>=FG_OVERFLOW_THRESHOLD||delta_q<=-FG_OVERFLOW_THRESHOLD){
		printk(KERN_DEBUG "OVERFLOW delta_q=%d\n",delta_q);
		delta_q=0;
...
//		A possible filter to the voltage - but might prevent a shutdown when the device's voltage rapidly drops
//		di->voltage_mV = filter_voltage(&di->cell,di->voltage_mV);
		fg_process(&di->cell, delta_q, di->raw_voltage_mV, di->voltage_mV,
	...
	printk(KERN_DEBUG "fg_process: soc=%d, voltage_norm=%d, acc_q=%d, delta_q=%d, voltage_raw=%d, current_now=%d, current_avg=%d, temperature_C=%d\n",
							di->cell.soc, di->voltage_mV, acc_q/5, delta_q, di->raw_voltage_mV, (int16_t)(di->current_uA/1000), (int16_t)(di->current_avg_uA/1000), di->temp_C);
#endif
#else
...
#ifdef CONFIG_MACH_OMAP4_JET //For Jet, we want to continue provide power from usb port and not battery
	if(di->charge_full_flag == true) {
		dev_dbg(di->dev, "Charger: charge_full_flag true\n");
		if (time_after(jiffies, di->charge_full_time + HZ )) {
			di->charge_full_flag = false;
			//Change VSYSREG to default
			twl_i2c_write_u8(TWL6032_MODULE_CHARGER, VSYSREG_3P54,
						CHARGERUSB_VSYSREG);
			dev_dbg(di->dev, "Charger: VSYSREG reset\n");
		}
	}
#else
	/* Stop the charger */
...
static void twl6030_bci_battery_work(struct work_struct *work)
{
	...
	/* for JET_SUN and SNOW di->voltage_mV is updated by twl6030_current_avg()*/
	...
	di->temp_C = adc_to_temperature(di, adc_code);

	/* Generate battery temperature event (HEALTH=Overheat) */
	if((di->temp_C < -400) && (di->bat_health == POWER_SUPPLY_HEALTH_GOOD))
	{
			temp1 = twl6032_get_battery_temperature(di);
			temp2 = twl6032_get_battery_temperature(di);
			if(temp1 < -400 && temp2 < -400)
			{
				di->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
				di->bat_health = POWER_SUPPLY_HEALTH_OVERHEAT;
				power_supply_changed(&di->bat);
...
static void twl6030_bci_battery_external_power_changed(struct power_supply *psy)
{
	...
	/* Do not reset the scheduler during discharge.
	 * It right away calls fg_process, instead of 10sec, and wronly impacts FG capacity calculation.
	 */
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		/*for JET_SUN and SNOW di->voltage_mV is updated by twl6030_current_avg() */
#ifdef CONFIG_FUEL_GAUGE
		val->intval = (di->cell.av_voltage) * 1000;
#else
		val->intval = di->voltage_mV * 1000;
#endif
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		/*We just want waht driver report, so no need to remeasure the current*/
...
static int __devinit twl6030_bci_battery_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "BAT Bootup Voltage:%d \n", di->voltage_mV);
#if defined(CONFIG_MACH_OMAP4_JET)
	...
	if (!is_battery_present(di)) {
		...
		twl_i2c_read_u8(TWL6032_MODULE_CHARGER, &reg, CHARGERUSB_INT_STATUS);

		fault = !(di->stat1 & CONTROLLER_STAT1_LINCH_GATED) && !(di->stat1 & CONTROLLER_STAT1_FAULT_WDG);
		charge_usb = (di->stat1 & VBUS_DET);
		if(charge_usb && (reg & CHARGERUSB_FAULT)) { // USB is connected, and there might be an error
			u8 usb_charge_sts1 = 0;
			twl_i2c_read_u8(TWL6030_MODULE_CHARGER, &usb_charge_sts1, CHARGERUSB_STATUS_INT1);
			if(usb_charge_sts1 && 0x7F) { // If bit 0 to 6 is high - we have an error
				charge_usb = 0;
				twl6030charger_fault_interrupt(0, di);
			}
		}
		charge_ac = (di->stat1 & VAC_DET) && !(di->stat1 & CONTROLLER_STAT1_EXTCHRG_STATZ);
...
#ifdef CONFIG_FUEL_GAUGE
	/*Parse u-boot voltage from boot command string */
	di->raw_voltage_mV = parse_cmdline_param("voltage=", 3620);	//u-boot default voltage 3620 mV (EDV1 at 25C)
	printk(KERN_INFO "uBoot Voltage = %d \n",di->raw_voltage_mV);

	/*Parse u-boot curent from boot command string */
	di->current_uA = parse_cmdline_param("current=", -230); //u-boot default current -230 mA

	/*Parse u-boot temperature from boot command string */
	di->temp_C = parse_cmdline_param("temperature=", -400); //u-boot default temperature -40 degC

	di->voltage_mV = twl6030battery_calc_norm_voltage(di, di->raw_voltage_mV, di->current_uA, di->temp_C);

	printk(KERN_INFO "uBoot Voltage_norm = %d, uBoot Current = %d, temperature=%d\n",di->voltage_mV, di->current_uA, di->temp_C);

	if (is_battery_present(di)) {
		fg_init(&di->cell, di->voltage_mV, di->temp_C);
	} else {
		di->cell.cc = 1;
		di->cell.full = 1;
		power_supply_changed(&di->bat);
	...
	INIT_DELAYED_WORK_DEFERRABLE(&di->twl6030_bci_monitor_work, twl6030_bci_battery_work);
	INIT_DELAYED_WORK_DEFERRABLE(&di->twl6030_current_avg_work, twl6030_current_avg);
	...
	ret = twl6030print_status();
	if (ret)
		dev_dbg(&pdev->dev, "twl6030 status read failed\n");
