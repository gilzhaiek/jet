...
#ifdef REPORT_SUSPEND
static void android_suspend(struct usb_gadget *gadget)
{
	composite_suspend(gadget);
	/*
	 * In the JET devices, the suspend event only happen before the disconnect event,
	 * however the disconnect interrupt may miss sometimes after unplugging the usb calbe (JET-757).
	 * So the workaround is to disconnect in suspend state or report SUSPEND event to userspace and let
	 * high level handle. We choose the first appoach but need further investigation in future.
	 */
