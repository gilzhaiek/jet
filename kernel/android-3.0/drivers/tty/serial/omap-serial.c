...
#ifndef CONFIG_CONSOLE_DEBUG
// UART port that is only used for console
...
#ifndef CONFIG_CONSOLE_DEBUG
	// For the snow and jet, UART_CONSOLE isn't used except for console, so skip it.
	if (pdev->id == UART_CONSOLE) {
		ret = 0;
		goto do_free;
	}
#endif
