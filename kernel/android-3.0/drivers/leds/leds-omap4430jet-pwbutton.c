...
#define LED_DEFAULT_VALUE 26
#define PW_LED_PWM2ON_DEFAULT_VALUE 0x72
#else
...
static void pwled_set_brightness(enum led_brightness value)
{
	...
	//pr_info("[%s:%u] value= %d, PWM2ON_PEROID= 0x%X (%d), brightnessPct= %d%%\n",
	//			__func__,__LINE__,(int)value,brightness,brightness, (value*100/255));
}
