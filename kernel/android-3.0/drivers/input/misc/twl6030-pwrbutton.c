...
#if CONFIG_MACH_OMAP4_JET//defined(CONFIG_JET_SUN) && defined(CONFIG_JET_V2)
...
static irqreturn_t powerbutton_irq(int irq, void *_pwr)
{
	...
	hw_state = twl6030_readb(pwr, TWL6030_MODULE_ID0, STS_HW_CONDITIONS);
	pwr_val = !(hw_state & PWR_PWRON_IRQ);
	//printk(KERN_INFO "%s:%d,%d,0x%x\n",__FUNCTION__,pwr_val,push_release_flag,prev_hw_state);
	if ((prev_hw_state != pwr_val) && (prev_hw_state != 0xFFFF)) {
		//printk(KERN_INFO "%s:1 event\n", __FUNCTION__);
		push_release_flag = 0;
		input_report_key(pwr->input_dev, pwr->report_key, pwr_val);
		input_sync(pwr->input_dev);
	} else if (!push_release_flag) {
		//printk(KERN_INFO "%s:2 events\n", __FUNCTION__);
		push_release_flag = 1;
		input_report_key(pwr->input_dev, pwr->report_key, !pwr_val);
		input_sync(pwr->input_dev);
