...
#define AM_ST_HIDDEN_REG	(0x02)
#define AM_OFFSET_X_H_M		(0x17)
...
//Note: ODR no effect in single conversion mode
...
#define M_HIGH_RESOLUTION	(0x60)

// Compass Temperature---------------------------
#define COMPASS_HIGH_RESOLUTION

#ifdef COMPASS_HIGH_RESOLUTION
#define COMPASS_CONFIG (M_ODR100|M_HIGH_RESOLUTION)
#else
#define COMPASS_CONFIG M_ODR100
#endif
...
// Bit masks-------------------------------------
#define MASK_CLR_0X03		(~0x03)
#define MASK_CLR_0XC0		(~0xC0)
...
#define CLICK_THS_VAL		(0x48) /* 2.3g @ 4g FS */

#define TIME_LIMIT			(0x3B)
#define TIME_LIMIT_VAL		(0x10) /* 40ms @ 400Hz */

#define TIME_LATENCY		(0x3C)
#define TIME_LATENCY_VAL	(0x20) /* 80ms @ 400Hz */

#define TIME_WINDOW			(0x3D)
#define TIME_WINDOW_VAL		(0xC8) /* 500ms @ 400Hz */

#define HP_CLICK_EN 		(1<<2) /* Enable bit for tap-tap high-pass filter */
#define A_HPM_NORMAL		(0x80) /* Normal mode for accelerometer high-pass filter */
...
#define TT_KEY_RECORD           2
#define TT_KEY_CAMERA           3
#define MAX_KEYPAD_CNT          4
unsigned int tap_tap_keypad_keycode_map[MAX_KEYPAD_CNT] = {KEY_ENTER, KEY_BACK, KEY_RECORD, KEY_CAMERA};
...
static struct lsm_status {
	...
	{
#ifdef CONFIG_JET_SENSORS_TAP_TAP
		/* When Tap-tap is enabled, accelerometer's output data rate should
		   be set to 400Hz regardless of the desired polling rate */
		.low_odr= A_ODR400,
		.high_odr= A_ODR400,
#else
		.low_odr= A_ODR100, /* Required for magnetometer ODR 100hz */
		.high_odr= A_ODR100,
#endif
		.threshold= 40,
...
int lsm9ds0_acc_hw_init(struct jet_sensors *jet_sensors)
{
	...
	// HP_CLICK_EN is used for tap-tap. Note that even if tap-tap is not enabled,
	// setting HP_CLICK_EN by default will not affect the non-tap-tap-related
	// functionalities of the 9-axis sensor.
	buffer[1]= FIFO_ENABLE | HP_CLICK_EN;
...
#ifdef CONFIG_JET_SENSORS_MAG_TEMPERATURE
static ssize_t set_temp_enable(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;
	u8 buffer[2];
	int ret;
	struct jet_sensors *jet_sensors = dev_get_drvdata(dev);

	if (strict_strtoul(buf, 10, &val) < 0)
		return -EINVAL;

	buffer[0] = AM_CNTRL5_ADDR;
	if(val)
	{
		buffer[1]=COMPASS_CONFIG|MAG_TEMP_EN;
	}
	else
	{
		buffer[1]=COMPASS_CONFIG;
	}

	ret= jet_i2c_write(jet_sensors,ACC_MAG_ADDRESS, buffer, 2);
	CHECK_ERR_RETURN(ret);

/*As ST email, we also need to do sequence below to enable temperature sense*/
/*
1.       I2C_Write(0x02, 0x10);
2.       a = I2C_read(0x17);
3.       I2C_Write(0x17, (a | 0x04));
4.       I2C_Write(0x02, 0x00);
*/
	buffer[0] = AM_ST_HIDDEN_REG;
	buffer[1]= 0x10;
	ret= jet_i2c_write(jet_sensors,ACC_MAG_ADDRESS, buffer, 2);
	CHECK_ERR_RETURN(ret);

	buffer[0] = AM_OFFSET_X_H_M;
	ret=jet_i2c_read(jet_sensors, ACC_MAG_ADDRESS,
		buffer[0], &buffer[1], 1);
	CHECK_ERR_RETURN(ret);
...
static ssize_t show_mag_temp(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	ret=jet_i2c_read(jet_sensors, ACC_MAG_ADDRESS,
		(OUT_TEMP_L_XM|I2C_AUTO_INCREMENT), (u8*)&temp_raw, 2);
	CHECK_ERR_RETURN(ret);
#if LOCAL_DEBUG
	printk("data=0x%.4X, 0x%.2X, 0x%.2X\n",(u16)temp_raw, (u8)temp_raw, (u8)(temp_raw>>8));
#endif
	/* In order to get real Celsius degree change, we need
	 * temp_raw/(8), also need to consider "two's complement data in 12-bit format, right justified"
	 * But we want to check raw data first
	*/
	return sprintf(buf, "%d\n",temp_raw);
}

DEVICE_ATTR(mag_temp_enable, S_IWUGO|S_IRUGO, NULL, set_temp_enable);
DEVICE_ATTR(mag_temp, S_IWUSR|S_IRUGO, show_mag_temp, NULL);
#endif
							...
							unsigned char mode, unsigned int ms_delay)
{
	...
	if(flag){
		...
		buffer[1]=(buffer[1] & MASK_CLR_0X03)|M_CONTINUOUS;
		/*NOTE: If fast polling(max 100Hz for current compass) is required
		* the accelerometer must be in power-down mode or ODR>=100.
		* See data sheet Page 58 note
		*/
	}
	else{
		buffer[1]=(buffer[1] & MASK_CLR_0X03)|M_PM_DOWN;
		mag_ms_delay=1000;//default setting
	}
...
int lsm9ds0_tap_tap_hw_init(struct jet_sensors *jet_sensors) {
	...
	// Set the value of CLICK_CFG to configure which axis to monitor for tap-tap
	// and whether to detect single or double-tap for the selected axes.
	buffer[0] = CLICK_CFG;
	buffer[1] = CLICK_CFG_VAL;
	err = jet_i2c_write(jet_sensors, ACC_MAG_ADDRESS, buffer, 2);
	if(err)	goto err1;

	// Set the threshold, time limit, time latency and window values for tap-tap.
	...
	// Read the contents of AM_CNTRL7_ADDR
	buffer[0] = AM_CNTRL7_ADDR;
	err=jet_i2c_read(jet_sensors, ACC_MAG_ADDRESS, buffer[0], &buffer[1], 1);
	if(err) goto err1;
	
	// Set the accelerometer's high pass filter mode to normal mode in AM_CNTRL7_ADDR
	buffer[1] = (buffer[1] & MASK_CLR_0XC0) | A_HPM_NORMAL;
	err=jet_i2c_write(jet_sensors, ACC_MAG_ADDRESS, buffer, 2);
	if(err) goto err1;
...
//		if(buffer & CLICK_SRC_X_BIT) { printk(KERN_INFO "CLICK_SRC_X_BIT\n");}
		if((buffer & CLICK_SRC_DCLICK_BIT) && (buffer & CLICK_SRC_Z_BIT)) {
			...
			tap_tap_click_run(jet_sensors->tap_tap_input_dev, TT_KEY_CAMERA);
		}
	}
}
#endif
...
int lsm9ds0_freefall_hw_init(struct jet_sensors *jet_sensors)
{
    ...
    /* 8 corresponds to 80ms for 100hz odr */
	buffer[2] = 0x08; //1/ODR
