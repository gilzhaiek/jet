...
//#define APDS_DEBUG
#ifdef APDS_DEBUG
#define APDS_PRINT(fmt, args...)  printk(KERN_INFO "[%s:%u]" fmt,__FUNCTION__,__LINE__,##args)
#else
#define APDS_PRINT(fmt, args...)
#endif
...
static ssize_t apds_write(struct file *filp, const char __user *buff, size_t count,loff_t *f_pos)
{
	...
	if(count==1){
		...
		if(ret==0){
			...
			APDS_PRINT("mode=%d ret=%d tmp=%p count=%d\n",(int)mode, ret, tmp,count);
			switch (mode)
			{
				case 6:	// # echo 6 > /dev/apds9900
					ret = apds9900_light_hw_init(apds);
					APDS_PRINT("INIT APDS9900 ret=%d\n",ret);
					ret = 2;
				break;
				case 7:	// # echo 7 > /dev/apds9900
					ret = apds9900_als_enable(apds);
					APDS_PRINT("Enable ALS ret=%d\n",ret);
					ret = 2;
				break;
				case 8:	// # echo 8 > /dev/apds9900
					ret = apds9900_als_disable(apds);
					APDS_PRINT("Disable ALS ret=%d\n",ret);
					ret = 2;
				break;
				case 9:	// # echo 9 > /dev/apds9900
					ret = apds9900_als_read(apds, buffer, 4);
					APDS_PRINT("Read ALS ret=%d \n",ret);
					APDS_PRINT("buffer[0]=0x%X  buffer[1]=0x%X buffer[2]=0x%X buffer[3]=0x%X\n",buffer[0],buffer[1],buffer[2],buffer[3]);
					ret = 2;
				break;
				default:
...
static long apds_ioctl(struct file *filp,unsigned int cmd, unsigned long arg)
{
	...
	if(ret)
	{
		printk(KERN_ERR "%s: mutex_lock_interruptible returned %d\n",__func__, ret);
		return ret;
	}
