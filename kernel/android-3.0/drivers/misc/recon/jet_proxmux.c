...
)
{
        ...
        (cmd == PROXMUX_IOCTL_GET_STATUS) || (cmd == PROXMUX_IOCTL_SET_MODE)  || (cmd == PROXMUX_IOCTL_SET_MIN_DELAY) )
   {
      ...
      if ( (pnq) && ( (cmd != PROXMUX_IOCTL_SET_ENABLE) && (cmd != PROXMUX_IOCTL_GET_STATUS) ) ) 
      {
          ...
          //Because the IRQ sensor not required to have delay time, so we return positive code to userspace
          return NO_DELAY_SETTING;
      }
...
RI_SENSOR_STATUS set_sensor_reporting_rate (struct _sensnode* pn, struct _sensworkqueue* pwq, unsigned int delay_ms)
{
   ...
   if (pwq != 0)
   {
      ...
      //If Sensors are grouped in the custom Kernel queue, no need to have delay time set separately, so we return some positive code to userspace
      return NO_DELAY_SETTING_FOR_SENSOR_QUEUE;
   }
