...
#ifdef CONFIG_JET_KEYS
#define GPIO_JET_BUTTON      143
#endif
...
static struct panel_kopin_a230_data kopin_data;
...
static void jet_camera_init(void)
{
	...
	printk(KERN_INFO "%s:set rate return %d,0x%p\n", __FUNCTION__,ret,src_clk);
#ifdef CAMERA_INIT_ENABLE
	clk_enable(src_clk);
#else
	//By default the clock not on
	//clk_disable(src_clk);
#endif
...
#ifdef CONFIG_JET_KEYS
static struct platform_device jet_keys_device = {
	.name = "jet-keys",
	.id = -1,
};
#endif
...
/* Initializes the OMAP dpll_per_m3x2 clock to 192MHz. The output of this clock is shared
 * by the camera and the Kopin a230. Both circuits expect this clock to be 192MHz. */
static void dpll_per_m3x2_clk_init(void)
{
	struct clk *dpll_per_m3x2_clk;
	int ret;
	
	dpll_per_m3x2_clk = clk_get(NULL, "dpll_per_m3x2_ck");
	if (IS_ERR(dpll_per_m3x2_clk)) {
		pr_err("Cannot request dpll_per_m3x2_ck\n");
		goto clk_busy;
	}

	ret = clk_set_rate(dpll_per_m3x2_clk, 192000000);
	if(ret<0){
...
#ifdef CONFIG_JET_KEYS
	omap_mux_init_signal("uart3_rx_irrx.gpio_143", OMAP_MUX_MODE3|OMAP_PIN_INPUT);
	/*external pull up*/
	//omap_mux_init_gpio(GPIO_JET_BUTTON, OMAP_PIN_INPUT | OMAP_PIN_OFF_WAKEUPENABLE);
	/*internel pull up*/
	omap_mux_init_gpio(GPIO_JET_BUTTON, OMAP_PIN_INPUT_PULLUP | OMAP_PIN_OFF_WAKEUPENABLE);
#endif
#if 0//test prupose
...
//Kopin setting pins--------------------------------
	...
	gpio_direction_output(A230_POWER_ENABLE_GPIO, 0);//disable kopin for kernel boot time
#else
...
/* Initializes the OMAP fref_clk4. The output of this clock is used by the Kopin a230. The
 * clock rate is set to 27.428571MHz (192MHz/7) to minimize harmonic interference with the
 * GPS antenna */
static void kopin_a230_clk_init(void)
{
	struct clk *src_clk, *parent_clk, *phy_ref_clk;
	int ret;
	...
	src_clk = clk_get(NULL, "auxclk4_src_ck");
	if (IS_ERR(src_clk)) {
		pr_err("Cannot request source clock\n");
		goto src_clk_busy;
	}
	...
	if (IS_ERR(phy_ref_clk)) {
		pr_err("Cannot request auxclk4_ck\n");
		goto aux_clk_busy;
	}

	ret = clk_set_rate(phy_ref_clk, 27428571);
	printk(KERN_INFO "%s:set rate return %d,0x%p\n", __FUNCTION__,ret,src_clk);
...
src_clk_busy:
	...
	{
		/* Save the clock to be used later in the Kopin a230 driver */
		kopin_data.src_clk = src_clk;
	}
}
