...
int omap4_charger_detect(void)
{
	...
	do {
		...
		printk(KERN_INFO "CONTROL_USB2PHYCORE=0x%X, chargertype=%d\n",usb2phycore,chargertype);
#endif
		if (chargertype){
...
#ifdef CONFIG_POWER_SUPPLY_DEBUG
	printk(KERN_INFO "final CONTROL_USB2PHYCORE=0x%X\n",usb2phycore);
#endif
	...
	case CHARGER_TYPE_APPLE:
		...
		pr_info("[%s:%u] Apple detected\n",__FUNCTION__,__LINE__);
		break;
	case CHARGER_TYPE_DEDICATED:
		charger = POWER_SUPPLY_TYPE_USB_DCP;
		pr_info("[%s:%u] DCP detected\n",__FUNCTION__,__LINE__);
		break;
	case CHARGER_TYPE_HOST:
		charger = POWER_SUPPLY_TYPE_USB_CDP;
		pr_info("[%s:%u] CDP detected\n",__FUNCTION__,__LINE__);
		break;
	case CHARGER_TYPE_PC:
		charger = POWER_SUPPLY_TYPE_USB;
		pr_info("[%s:%u] PC detected\n",__FUNCTION__,__LINE__);
		break;
	case CHARGER_TYPE_PS2:
		pr_info("[%s:%u] PS/2 detected!\n",__FUNCTION__,__LINE__);
		break;
	default:
		pr_err("[%s:%u] Unknown charger detected! %d\n", __FUNCTION__,__LINE__,chargertype);
