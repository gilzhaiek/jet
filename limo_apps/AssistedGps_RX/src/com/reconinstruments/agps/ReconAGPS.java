...
{   
	/* Nested sub-classes; data structure for easier argument passing */
	 public class LocationFix
	 {
		 ...
		 public int flags = 0;            // location flags bitmask; values must match GpsLocationProvider.java
	 ...
	 public class AssistanceData
	 {
		 ...
		 public String      almanac_file;
		 public int  almanac_format = 0;
		 public String      rinex_file;
		 public String[]    raw_ephemeris;
		 public int         ephemeris_format = 0;
		 
		 public int flags = 0;         // bitmask of assistance types client is providing
         ...
         /* ephemeris formats */
         public static final int EPHEMERIS_FORMAT_RINEX = 1;
         public static final int EPHEMERIS_FORMAT_RAW = 2;
	...
	private static final String TAG = "ReconAGPS.Java";
    ...
    /* Callback Type Flags */
	private static final int GPS_COMMAND_FLAG = 0x01;
	private static final int GPS_STATUS_FLAG  = 0x02;
    ...
    public static final int STATUS_GPS_REQUEST_ASSIST  = 0x05;
    public static final int STATUS_GPS_DEL_ASSIST      = 0x06;
	...
	private int     mRawEphIndex;                    // offset into raw ephemeris array, as these go 1 by 1
	...
	protected static ReconAGPS mInstance = null;
	 ...
	 public  static ReconAGPS Initialize () throws UnsatisfiedLinkError, IOException
	 {
		 if (ReconAGPS.mInstance == null)
		 {
			 ...
			 ReconAGPS.mInstance.mRawEphIndex = 0;
		 }
	 ...
	 public int Start (IReconGpsEventListener cbk)
	 {
		 // remember client callback
		 m_client = cbk;
		 
		 int result = GpsNative.agps_init(
				 ReconAGPS.HANDLER_CLASS, ReconAGPS.STATUS_CALLBACK, ReconAGPS.COMMAND_CALLBACK);
	 ...
	 /* Status Report */
	 static void statusReport (int status, int extra)
	 {
		String strLog = String.format("+++ GPS Status callback. Status: [0x%x], Extra: [0x%x] +++\n", status, extra);
		Log.d(TAG, strLog);
	 ...
	 private class GpsMessageHandler extends Handler
	 {
	    ...
	    public void handleMessage(Message msg)
	    {
	    	...
	    	switch (msg.what)
	    	{
	    	    ...
	    	    {
                    if (m_client != null)
                        m_client.onStatusReceived(msg.arg1, msg.arg2);
	    	    }
	    	    break;
     ...
     private void continueAssistance(int status)
     {
    	 ...
    	 {  
	    	 ...
	    	 switch (mAssistCommand)
	    	 {
	    	     ...
	    	     {
	    	    	 // must allow for sending array of Raw Ephemeris files
	    	    	 if (mAssistData.ephemeris_format == AssistanceData.EPHEMERIS_FORMAT_RINEX)
	    	    	   mAssistCommand = ReconAGPS.COMMAND_GPS_END_ASSIST;
	    	    	 else
	    	    	 {
	    	    		 mRawEphIndex++;
	    	    		 if (mRawEphIndex == mAssistData.raw_ephemeris.length)
	    	    			 mAssistCommand = ReconAGPS.COMMAND_GPS_END_ASSIST;
	    	    	 }
	    	     }
	    	     break;
	    	     ...
	    	     case ReconAGPS.COMMAND_GPS_END_ASSIST:
	    	     {
	    	    	  ...
	    	    	  // Client should now get FIX_START!
	    	    	  mAssistData = null;
    		          mAssistCommand = 0;
    		          mRawEphIndex = 0;
    	 ...
    	 else if (mAssistCommand == ReconAGPS.COMMAND_GPS_INJECT_ALMANAC)
    	 {
    		 ...
    		 result = GpsNative.agps_inject_almanac(mAssistData.almanac_file, mAssistData.almanac_format);
    			  ...
    			  (mAssistData.ephemeris_format == mAssistData.EPHEMERIS_FORMAT_RINEX) )
    	 {
    		 Log.i(TAG, "Injecting Ephemeris Assistance ...");
    		 result = GpsNative.agps_inject_ephemeris(mAssistData.rinex_file, mAssistData.ephemeris_format);
    	 }
    	 
    	 else if ( (mAssistCommand == ReconAGPS.COMMAND_GPS_INJECT_EPHEMERIS) &&
    			  (mAssistData.ephemeris_format == mAssistData.EPHEMERIS_FORMAT_RAW) )
    	 {
    		 Log.i(TAG, "Injecting RAW Ephemeris Assistance (RAW File: " + mAssistData.raw_ephemeris[mRawEphIndex] + ") ...");
    		 result = GpsNative.agps_inject_ephemeris(mAssistData.raw_ephemeris[mRawEphIndex], mAssistData.ephemeris_format);
    	 }
    	 ...
    	 else if (mAssistCommand == ReconAGPS.COMMAND_GPS_END_ASSIST)
    	 {
    		 ...
    		 // client will now get COMMAND_GPS_END_ASSIST result callback
    	 }
