...
--specified for the Jet platform only, the most recent messages group by group and category
CREATE VIEW recent_messages_by_group_and_category AS
SELECT *
FROM
	(SELECT last_messages_by_category._id, Timestamp, Text, Processed, Description, PressAction, PressCaption, CategoryID, GroupID
	FROM last_messages_by_category,MessageCategories
	WHERE last_messages_by_category.CategoryID=MessageCategories._id)
GROUP BY GroupID, Description
