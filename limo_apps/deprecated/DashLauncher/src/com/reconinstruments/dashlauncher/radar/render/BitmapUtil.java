...
public class BitmapUtil
{
	...
	public static Bitmap getGlyph(String glyph, int fontSize)
	{
		...
		//Log.i(TAG, "getGlyph('" + glyph + "', " + fontSize + ")");
	...
	public static Bitmap getAsciiChar(String glyph, int fontSize)
	{
		if (!IsInitialized)
			InitializePaints();
		...
		int width  = Math.max(4, tBounds.width() + 1);
		int height = fontSize + 6;

		mGenSurface = Bitmap.createBitmap(width, height, Config.ARGB_8888);
		mGenCanvas = new Canvas(mGenSurface);

		mGenCanvas.drawText(glyph, (width - textWidth/2), height - 6, mGlyphPaint);
		return mGenSurface;
	}   
}
