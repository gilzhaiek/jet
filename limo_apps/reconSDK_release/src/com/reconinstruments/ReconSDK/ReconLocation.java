...
/** Concrete Implementation of {@link ReconEvent#TYPE_LOCATION} Data Type <p>
 * 
 *  ReconLocation Data Object encapsulates Position on MOD Live Platform. <p>
 *  Location changes are not broadcasted by MOD Server.  Client must explicitly
 *  request an update {@link ReconSDKManager#receiveData(IReconDataReceiver, int)}<p>
 *  If GPS fix is not available, retrieved values will be set to {@link ReconLocation#INVALID_LOCATION}
 *  */
public class ReconLocation extends ReconEvent
{
    ...
    /** Previous Location Accessor
     * <p>
     * @return Previous Location Value
     */
	...
	protected void fromBundle(Bundle b) throws DataFormatException
	{
		...
		// depending on whether device had GPS fix or not
		 ...
		 if (b != null)
		 {
			 if (b.containsKey(ReconMODService.FieldName.LOCATION_VALUE) == true)
				 mLocation         = (Location)b.getParcelable(ReconMODService.FieldName.LOCATION_VALUE);
			
			 if (b.containsKey(ReconMODService.FieldName.LOCATION_PREVIOUS) == true)
		 ...
		 if ( (mLocation == ReconLocation.INVALID_LOCATION) || (mPreviousLocation == ReconLocation.INVALID_LOCATION) )
		 {
			 ...
			 Log.i(TAG, "GPS Fix not resolved on MOD Server");
