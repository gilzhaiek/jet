	...
	class Drawable extends AbstractBitmapDrawable {
		...
		public void draw(Canvas canvas) {
			...
			if(landscape){
				...
				Bitmap bitmap = Bitmap.createBitmap(bitmapPixels, framebufferwidth, framebufferheight, Config.RGB_565);
				
				Matrix matrix = new Matrix();
				//matrix.setTranslate(0, 120);
				matrix.postScale(428.0f/240.0f,428.0f/240.0f);
				matrix.postRotate(-90,0,0);
				matrix.postTranslate(-473, 428*2);
				//matrix.postTranslate(-473, 800);
				//matrix.postRotate(-90,0,0);
				
				canvas.drawBitmap(bitmap, matrix, null);

			}
			else if (vncCanvas.getScaleType() == ImageView.ScaleType.FIT_CENTER)
			{
				canvas.drawBitmap(bitmapPixels, 0, framebufferwidth, xoffset, yoffset, framebufferwidth, framebufferheight, false, null);
			}
			else
			{
