...
//============ parcelable protocol handlers
    ...
    public void writeToParcel(Parcel _parcel, int flags) {		// data out (encoding)
    	_parcel.writeValue(mCenterPoint);
    ...
    private GeoRegion(Parcel _parcel) {							// constructor from parcel - data in (decoding)
    	mCenterPoint = (PointXY) _parcel.readValue(getClass().getClassLoader());
    	mCenterOnUser = (_parcel.readInt() == 1 ? true : false);
    	mBoundingBox = (RectXY) _parcel.readValue(getClass().getClassLoader());
    }

    public void ChangeHeightToFit(float desiredHeightInM) {
       float heightDegrees = (float)(desiredHeightInM / meridonalCircumference * 360.0f);    
       mBoundingBox = new RectXY(mBoundingBox.left, mCenterPoint.y+heightDegrees/2.f, mBoundingBox.right, mCenterPoint.y-heightDegrees/2.f);
       mSize = new PointXY(mBoundingBox.right-mBoundingBox.left, mBoundingBox.top-mBoundingBox.bottom); 
    }

    public void ChangeWidthToFit(float desiredWidthInM) {
        //float widthDegrees = (float)(desiredWidthInM / meridonalCircumference * 360.0f);
        float widthDegrees = (float)(desiredWidthInM * 360.0f /(equitorialCircumference*Math.cos(Math.toRadians(mCenterPoint.y))) ); 
        mBoundingBox = new RectXY(mCenterPoint.x-widthDegrees/2.f, mBoundingBox.top, mCenterPoint.x+widthDegrees/2.f, mBoundingBox.bottom);
        mSize = new PointXY(mBoundingBox.right-mBoundingBox.left, mBoundingBox.top-mBoundingBox.bottom); 
    }

    public void MapToWidthHeightRatio(double desiredAspectRatio) {
        // TODO Auto-generated method stub
        float height = heightInMeters();
        float width = widthInMeters();
        
        if((width/height) > desiredAspectRatio) { // height is too small
            ChangeHeightToFit((float) (width / desiredAspectRatio));
        }
        else { // height is too smalls
            ChangeWidthToFit((float) (height * desiredAspectRatio));
        }
    }
