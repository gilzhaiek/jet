...
import com.reconinstruments.reconsensor.SensorStatus;


/**
 *  <code>JetJumpAnalyzer</code> deals with the complexity of
 *  detecting jumps. The heavylifting is done by
 *  <code>JetJumpEngine</code>. This class is more like a state
 *  machine talkign with sensors and etc.
 *
 */
public class JetJumpAnalyzer implements SensorEventListener 
{
	// standard ident tag
	private static final String TAG = "RECON.JUMP.JumpAnalyzer";
	private static boolean DEBUG = ReconJetJumpManager.DEBUG; 
	private int mQueue_rate_ms = 0;              // fast queue rate; always adjusted on slowest sensor
  
    // singleton pattern
    static  JetJumpAnalyzer  m_instance = null; 
    private JetJumpAnalyzer () {}
  
    public static JetJumpAnalyzer Instance(Context c) 
    {
    	if (m_instance == null)
        {
    		m_instance = new JetJumpAnalyzer ();
       	    ...
       	    try 
		    {
                ...
                else
                {
            	    ...
            	    m_instance.mEngine = new JetJumpEngine();
                }
    ...
    private JetJumpEngine         mEngine;           // raw algorithm: Gory details
   ...
   /**
    * FailSafe Activated, do reset as if jumps were detected
    */
   public void onFailSafeActivated() {
	   if (DEBUG) {
			JetJumpDebugLog.startLine(System.currentTimeMillis());
			JetJumpDebugLog.append("Resetted due to Jump FailSafe");
   ...
   {	   
      ...
      if (mEngine.AirState().equals (JetJumpEngine.AirState.LANDED) )
         return;
      ...
      {
    	  JetJumpEngine.AirState airState = mEngine.processAccelerometer(event.values);
    	  if (airState == JetJumpEngine.AirState.LANDED)
    	  {
    		  
    		  // Stops Failsafe Timer
    		  mIJumpEnd.stopTimer();
    		  ...
    		  if (DEBUG && jump != null) { // Log Jump Data
  				JetJumpDebugLog.startLine(jump.mDate);
  				JetJumpDebugLog.append("Jump Landed");
  				JetJumpDebugLog.endLine();
  			  }
