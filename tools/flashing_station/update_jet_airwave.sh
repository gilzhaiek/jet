...
VERSION_1_OMAP4_PATH=${CLEAN_PATH}/omap4_emmc_files_jet_version_1
...
VERSION_1_UBOOT_FILE=${VERSION_1_OMAP4_PATH}/u-boot.bin
...
TARGET_UPDATE_FILE=/mnt/sdcard/ReconApps/cache/${UPDATE_FILE_NAME}
CLEAN_NEW_IMAGE=${CLEAN_IMAGES_PATH}/${NEW_IMAGE_FILE_NAME}
INC_NEW_IMAGE=${INC_IMAGES_PATH}/${NEW_IMAGE_FILE_NAME}
SUN_NEW_IMAGE=${SUN_OMAP4_PATH}/${NEW_IMAGE_FILE_NAME}
...
function update_device_serial() {
  export device_serial=`${ADB} devices | grep '015D\|26\|27\|28' | sed -e 's|device||g;s| ||g'`
}
...
function wait_for_full_boot() {
  ...
  while true; do
    ...
    if [ `echo $adb_devices | grep -c '015D\|26\|27\|28'` -eq 1 ]; then
...
function read_serial_from_user {
  ...
  if [[ ! ${#user_serial} -eq $1 ]]; then
    echo "Serial must be excatlly $1 chars, Please try again:";
...
function read_serial {
  while true; do
    ...
    case $ans in
      ...
      [Rr]* ) read_serial_from_user 2; new_serial=2700300${user_serial}; break;;
...
if [[ "$device_serial" == *015D* ]] || [[ "$device_serial" == *26* ]] ||[[ "$device_serial" == *27* ]] || [[ "$device_serial" == *28* ]]; then
  echo -e " Device Serial\t= \033[1;32m$device_serial\033[m"
else
...
while true; do
  echo -e "Would you like to revert to Factory Version to test the update (Snow2 Only!):"
  echo -e " - [\033[1;32m1\033[m] Revert to Version 1 (July 24th)"
  echo -e " - [\033[1;32mC\033[m]ontinue to update process"
  read -n1 ans
  echo " ";
  case $ans in
    [1]* ) omap4_path=${VERSION_1_OMAP4_PATH}; uboot_file=${VERSION_1_UBOOT_FILE}; reflash_device; break;;
    [Cc]* ) break;;
    * ) echo "Please answer 1/C";;
  esac
done
