...
#define GRAVITY_EARTH (1.0f)   // from google sensors.h
#define ACC_FACTOR   1.0f
#define GYRO_FACTOR  1.0f
#define MAG_FACTOR   1.0f
    ...
    while (processed < readsize)
    {
        ...
        while (dataoffset < datasize)
        {
            if (handle == RI_SENSOR_HANDLE_ACCELEROMETER)
            {
                ...
                v = (short*)(databuffer + processed + dataoffset); xf = *v; xf = ((xf * ACC_FACTOR * GRAVITY_EARTH) / 1.0f);
                v = (short*)(databuffer + processed + dataoffset + sizeof(short) ); yf = *v; yf = ((yf * ACC_FACTOR * GRAVITY_EARTH) / 1.0f);
                v = (short*)(databuffer + processed + dataoffset + sizeof(short) * 2); zf = *v; zf = ((zf * ACC_FACTOR * GRAVITY_EARTH) / 1.0f);

                printf("reconacc,%.0f,%.0f,%.0f\n", xf, yf, zf);
