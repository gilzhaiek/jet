...
public class MtpService extends Service {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_USER_PRESENT.equals(action) || action.equals("com.reconinstruments.MTP_ENABLE")) {
                Log.d(TAG, "Received " + action + " - mMtpDisabled: " + mMtpDisabled);
                synchronized (mBinder) {
                    if (mMtpDisabled) {
                        // Recreate MtpServer to enable MTP
    ...
    public void onCreate() {
        ...
        mContext = this;
        try {
            // Determine if MTP service should be enabled at this point. There are three cases in which MTP should be
            // disabled here:
            //  1) Recon_boot.sh script not finish. Note: This is the part of device boot sequence.
            //  2) User is currently in the lock screen (via check of PASSCODE_IN_LOCK)
            //  3) User has enabled passcode but the lock screen has not yet been triggered. This occurs on boot up if
            //     the usb is plugged in before the lock screen has been triggered. (via checking PASSCODE_ENABLED setting
            //     and sys.reconinstruments.lockedonce system property, respectively).
            // If these two cases are not met, then MTP will be enabled as normal.
            if (!SystemProperties.get("recon_boot.done", "").equals("1")){
                mMtpDisabled = true;
            } else if ((Settings.System.getInt(getContentResolver(), "com.reconinstruments.settings.PASSCODE_IN_LOCK") == 1)) {
                mMtpDisabled = true;
            } else if (((Settings.System.getInt(getContentResolver(), "com.reconinstruments.settings.PASSCODE_ENABLED") == 1) &&
                SystemProperties.get("sys.reconinstruments.lockedonce").equals(""))) {
                mMtpDisabled = true;
            } else {
                mMtpDisabled = false;
            }
        } catch (Settings.SettingNotFoundException e) {
            Log.e(TAG, "Failed to retrieve com.reconinstruments.settings.PASSCODE_ENABLED");
            mMtpDisabled = false;
        }
        ...
        filter.addAction("com.reconinstruments.MTP_ENABLE");
