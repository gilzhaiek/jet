...
public final class BluetoothPairingDialog extends AlertActivity implements
    ...
    /* Patrick Cho - This is called when simple passkey confirmation is shown */
    private void createConfirmationDialog(CachedBluetoothDeviceManager deviceManager) {
        // Use custom Recon dialog for bluetooth pairing
        requestWindowFeature(Window.FEATURE_CUSTOM_TITLE);
        getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
        setContentView(R.layout.bluetooth_pair_req);
        getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.bluetooth_pair_title);

        // Set the passkey to the TextView
        TextView tv = (TextView)findViewById(R.id.passkey_number);
        tv.setText(mPairingKey);

        // Check build model. If it's Jet, then change the pair image to be Jet-specific
        ImageView iv = (ImageView)findViewById(R.id.pair_btn);
        if (Build.MODEL.equalsIgnoreCase("JET")) {
            iv.setImageResource(R.drawable.jet_select);
        }
    }
