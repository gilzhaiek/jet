...
void perform_post_charger_actions() {
    const char *pval;
    char usb_config[32];

    action_for_each_trigger("early-boot", action_add_queue_tail);
    action_for_each_trigger("boot", action_add_queue_tail);
    //set usb config properly
    pval = property_get("persist.sys.usb.config");
    if (pval) {
        strlcpy(usb_config, pval, sizeof(usb_config));
        property_set("sys.usb.config", usb_config);
    }
}

// returns true if the service that just died is "battery_charger"
bool is_charger_pid() {
    pid_t pid;
    int status;
    struct service *svc;
    ...
    if (!svc) {
        ERROR("[%s:%u] untracked pid %d exited\n", __FUNCTION__,__LINE__,pid);
        return false;
    }

    return !strcmp(svc->name, "battery_charger");
}
...
int main(int argc, char **argv)
{
    ...
    /*The reocn Snow 3.0 release never use charger phase, so change bootmoade from "charger"
    * to "reconcharger". In this way, the new uboot can support 3.0 release kernel
    * and android
    */
    is_charger = !strcmp(bootmode, "reconcharger");
    INFO("property init %d %s\n",is_charger,bootmode);
        ...
        for (i = 0; i < fd_count; i++) {
            if (ufds[i].revents == POLLIN) {
                if (ufds[i].fd == get_property_set_fd()) {
                    ...
                    if(is_charger && is_charger_pid()) {
                        perform_post_charger_actions();
                        is_charger = false;
                    }
