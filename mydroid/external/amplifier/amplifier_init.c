...
static int dspPatch(Tfa98xx_handle_t handle, const char* fileName)
{
	int ret;
	int fileSize;
	...
	print_verbose("Loading patch file %s\n", fileName);
...
int coldStartup(Tfa98xx_handle_t handle)
{
	Tfa98xx_Error_t err;
	...
	print_verbose("Waiting for IC to start up\n");
	err = Tfa98xx_ReadRegister16(handle, TFA98XX_STATUSREG, &status);
	print_if_false(err == Tfa98xx_Error_Ok);
	while ( (status & TFA98XX_STATUSREG_AREFS_MSK) == 0 && count>0)
	{
		/* not ok yet */
		err = Tfa98xx_ReadRegister16(handle, TFA98XX_STATUSREG, &status);
		...
		count--;
		//sleep(20);
		//print_verbose("Waiting for PLL lock\n");
	}
	if(count<=0)
		return -1;
	/*  powered on
	...
	* */
#if 1
	if(dspPatch(handle, LOCATION_FILES "coldboot.patch")!=0){
		__print_err_line;
		return -1;
	}
#else
...
static int setConfig(Tfa98xx_handle_t handle, const char* fileName)
{
	Tfa98xx_Error_t err;
...
static int setPreset(Tfa98xx_handle_t handle, const char* fileName)
{
	int ret;
	int presetSize;
	unsigned char* buffer;
...
static int setEQ(Tfa98xx_handle_t handle, const char* fileName)
{
	int ret;
	FILE* f;
	Tfa98xx_Error_t err;
	...
	while (!feof(f))
	{
		...
		if (ret == 6)
		{
			if ((b0 != 1) || (b1 != 0) || (b2 != 0) || (a1 != 0) || (a2 != 0)) {
				...
				print_if_false(err == Tfa98xx_Error_Ok);
				print_verbose("Loaded biquad %d\n", ind);
	  		} else {
				err = Tfa98xx_DspBiquad_Disable(handle, ind);
						print_if_false(err == Tfa98xx_Error_Ok);
						print_verbose("Disabled biquad %d\n", ind);
			}
		}
		else {
...
static void load_all_settings(Tfa98xx_handle_t handle, Tfa98xx_SpeakerParameters_t speakerBytes, const char* configFile, const char* presetFile, const char* eqFile)
{
	...
	/* load the settings */
	if(setConfig(handle, configFile)!=0)
		return;
	/* load a preset */
	if(setPreset(handle, presetFile)!=0)
		return;
	/* load an EQ file */
	setEQ(handle, eqFile);
}
...
#define WAIT_TRIES 1000
	...
	} else /* poll xmem for calibrate always */
	{
		...
		if(tries==WAIT_TRIES)
			print_verbose("calibrateDone 231 timedout\n");
	}
...
static void statusCheck(Tfa98xx_handle_t handle)
{
	...
	{
	  print_verbose("DSP watchDog triggerd");
	  return;
	}
}
...
static char initState=0;
/*
 * By defautl: use amplifier_init(1)
 * @param otcOn: 1= One time calibration; 0= claibrate every time when running amplifier_init
 */
int amplifier_init(int otcOn)
{
	Tfa98xx_Error_t err;
	Tfa98xx_handle_t handle;
	...
	{
		print_verbose("DSP not yet calibrated. Calibration will start.\n");
	...
	{
		print_verbose("DSP already calibrated. Calibration skipped and previous calibration results loaded from MTP.\n");
	}
	...
	* The MTPEX bit would be set and remain as 1 if MTPOTC is set to 1 */
	...
	print_verbose("Calibration value is %2.2f ohm\n", re25);
	...
	print_verbose("Kill child thread\n");
