...
#include "initTfa9887.h"

#define OPTIMIZED_RPC

/* 4 possible I2C addresses
 */
#define MAX_HANDLES 1//mono
        ...
        for (i = 0; i < MAX_HANDLES; ++i) {
                if (!handlesLocal[i].in_use) {
                        ...
                        case 0x6E:
                                ...
                                /* do a dummy read in order to generate some i2c clocks
                                 * when accessing the device for the first time
                                 */
                                Tfa98xx_ReadRegister16(i, TFA98XX_STATUSREG,
                                        &status);
                                error =
                                    Tfa98xx_ReadRegister16(i, TFA98XX_REVISIONNUMBER,
                                                           &rev);
...
Tfa98xx_Error_t Tfa98xx_DspSystemStable(Tfa98xx_handle_t handle, int *ready)
{
	...
	*ready = (status & (TFA98XX_STATUSREG_AREFS_MSK | TFA98XX_STATUSREG_CLKS_MSK))
	...
	/* if we get here we're fine, CF subsys is ready for access */
	*ready = 1;
                 ...
                 const unsigned char *bytes)
{
        ...
        while (index < length) {
                ...
                i2c_error = NXP_I2C_Write(handlesLocal[handle].slave_address, size, (unsigned char *)(bytes + index));
