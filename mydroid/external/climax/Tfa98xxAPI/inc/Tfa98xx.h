...
#define TFA98XX_API_REV_STR      "2.0"
#define TFA98XX_API_WAITRESULT_NTRIES        1000
...
typedef struct Tfa98xx_DBDrcStateInfo {
    float     GRhighDrc1[2];
    float     GRhighDrc2[2];
    float     GRlowDrc1[2];
    float     GRlowDrc2[2];
...
typedef struct Tfa98xx_StateInfo {
		...
		/* DBDRC state, when enabled */
		Tfa98xx_DBDrcStateInfo_t dbdrcState;
} Tfa98xx_StateInfo_t;
...
/* Check whether the DSP expects tCoef or tCoefA as last parameter in the speaker parameters
 * *pbSupportDBDrc=1 when DSP supports DBDRC, 
 * *pbSupportDBDrc=0 when DSP doesn't support it 
 */
...
typedef enum Tfa98xx_AgcGainInsert {
		Tfa98xx_AgcGainInsert_PreDBDrc = 0,
		Tfa98xx_AgcGainInsert_PostDBDrc,
} Tfa98xx_AgcGainInsert_t;
...
Tfa98xx_Error_t Tfa98xx_DspBiquad_SetCoeff(Tfa98xx_handle_t handle,
										   ...
										   float a2);
/* load all biquads.  The coefficients are in the same order als the Tfa98xx_DspBiquad_SetCoeff function b0, b1, ... */
