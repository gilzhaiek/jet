...
BTIPS_SCRIPTS_PATH=$MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/
...
#cp -Rfp $YOUR_PATH/dsp/system/lib/* $MYDROID/out/target/product/jet/system/lib
#cp -Rfp $YOUR_PATH/dsp/system/bin/* $MYDROID/out/target/product/jet/system/bin
...
echo "Copying root"
cp -Rfp $MYDROID/out/target/product/jet/root/* .

echo "Copying system"
cp -Rfp $MYDROID/out/target/product/jet/system/ .

echo "Copying data"
cp -Rfp $MYDROID/out/target/product/jet/data/ .

if [ "$PLATFORM_VERSION" = "2.2.1" ]; then
    if [ "$TARGET_PRODUCT" = "jet" ]; then
...
mkdir -p var/run
mkdir -p system/bin/
cp $MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/pan/*.sh data/btips/TI/scripts/
cp $MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/pan/dnsmasq system/bin/
cp $MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/pan/tcpdump system/bin/
cp $MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/pan/20-dns.conf system/etc/dhcpcd/dhcpcd-hooks/
cp $MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/pan/dhcpcd.conf system/etc/dhcpcd/
cp $MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/pan/dnsmasq_gn.conf system/etc/
cp $MYDROID/external/mcp/MCP_Common/Platform/scripts/android_jet/pan/dnsmasq_nap.conf system/etc/

echo "Setting up init.rc"
mv init.rc init.rc.bak
if [ "$PLATFORM_VERSION" = "2.2.1" ]; then
    if [ "$TARGET_PRODUCT" = "jet" ]; then
        cp -Rfp $BTIPS_SCRIPTS_PATH/init_2.2.1_jet.rc ./init.rc
    else
