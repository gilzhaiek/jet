...
#Copy makefiles to MYDROID, instead of private view.
cp -vf "$MCP_VIEW_PATH/MCP_Common/Platform/scripts/Android_root.mk" "$ANDROID_HOME/mydroid/external/mcp/Android.mk"
cp -vf "$MCP_VIEW_PATH/EBTIPS/apps/Android.mk" "$ANDROID_HOME/mydroid/external/mcp/EBTIPS/"
cp -vf "$MCP_VIEW_PATH/EBTIPS/apps/Android.mk" "$ANDROID_HOME/mydroid/external/mcp/B_TIPS/"

cd "$PWD" 

cd "$ANDROID_HOME/mydroid"

mkdir -p out/target/product/jet/obj/lib
mkdir -p out/target/product/jet/system/lib
...
if [ "$PLATFORM_VERSION" = "1.6" ]; then
    ...
    echo "UNKNOWN - Aborting (currently supporting Donut, Eclair(2.0 & 2.1), Froyo (2.2 & 2.2.1) and Gingerbread (2.3.3) only)"
		exit
fi
...
echo "mount_view.sh Done !!!"
echo "Done" >> $ANDROID_HOME/../mount_view.jet.no.gps.done
