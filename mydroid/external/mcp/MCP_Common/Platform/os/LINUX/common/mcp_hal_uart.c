...
#define LOG_TAG "MCP.MCP_HAL_UART"    // our identification for logcat (adb logcat MCP.MCP_HAL_UART:V *:S)
...
handle_t HAL_UART_Create (const handle_t hMcpf, const char *pDrvName)
{
	...
	{
		strncpy (pHalUart->cDrvName, pDrvName,sizeof(pHalUart->cDrvName)-1);/*Klocwork Changes  */
	}
	else
	{
                          ...
                          const McpU16              len)
{
	...
	halUartDestroy (pHalUart); /* Klocwork Changes */
    return status;
}
