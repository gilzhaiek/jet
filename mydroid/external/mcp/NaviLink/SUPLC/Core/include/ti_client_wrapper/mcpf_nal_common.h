 ...
 *******************************************************************************
  ...
  *
  * FileName         :   mcpf_nal_common.h
  *
  * Description      :   Common interface between MCPF and SUPL Core.
  *
...
#define MAX_CELL_PARAMS 5 //added for cellinfo  type
#define MAX_NMR_PARAMS 48
#define MAX_HSLP_PARAMS 2
...
typedef enum
{
	...
	MCPF_NAL_GET_MSISDN, /*Added for MSISDN*/
	MCPF_NAL_GET_HSLP, /*Added for AUTO HSLP*/ 
    MCPF_NAL_GET_SUBSCRIBER_ID_TYPE,
    MCPF_NAL_CMD_MAX
...
typedef struct createConnection{
    char a_hostPort[MAX_FQDN_LENGTH];
...
typedef struct getMSISDN{
	char a_msisdnData[MAX_SIZE_IMSI_BUFFER];
	int s32_size;
}s_NAL_msisdn;

typedef struct getHSLP{
	int a_hslpData[MAX_HSLP_PARAMS];
	int s32_size;
}s_NAL_hslp;

typedef struct gsmCellInfo{
    int a_cellInfo[MAX_CELL_PARAMS];
	int a_nmr[MAX_NMR_PARAMS];
