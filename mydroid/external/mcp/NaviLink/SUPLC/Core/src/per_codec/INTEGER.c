...
INTEGER_map_enum2value(asn_INTEGER_specifics_t *specs, const char *lstart, const char *lstop) {
	...
	el_found = (asn_INTEGER_enum_map_t *)b_search(&key,
...
INTEGER_map_value2enum(asn_INTEGER_specifics_t *specs, long value) {
	int count = specs ? specs->map_count : 0;
	if(!count) return 0;
	return (asn_INTEGER_enum_map_t *)b_search(&value, specs->value2enum,
...
INTEGER_decode_uper(asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
	...
	if(ct && ct->flags != APC_UNCONSTRAINED) {
		...
		if(ct->range_bits >= 0) {
			...
			//if(value < 0) _ASN_DECODE_STARVED; //rsuvorov change because we can receive negative value
