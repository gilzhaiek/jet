...
int hci_tty_open(struct inode *inod, struct file *file)
{
	...
	hst = (void*)kzalloc(sizeof(*hst), GFP_KERNEL);
		...
		loff_t *offset)
{
	...
	tout = wait_event_interruptible_timeout(hst->data_q,
			!skb_queue_empty(&hst->rx_list), msecs_to_jiffies(1000));
...
#if 0
		case TCFLSH:
			pr_debug("IOCTL TCFLSH invoked with %ld argument", arg);
			switch (arg) {
