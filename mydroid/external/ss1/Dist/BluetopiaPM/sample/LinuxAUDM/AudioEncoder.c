...
static int CmpTimespec(const struct timespec *TS1, const struct timespec *TS2)
{
   ...
   /* error.                                                            */
int InitializeAudioEncoder(unsigned int CallbackId, unsigned int SamplesPerSecond, unsigned int NumberOfChannels)
{
   int                                           Result;
   int                                           ret_val = -1;
   unsigned int                                  MTU;
...
int SendAudioData(void *Buffer, unsigned int Length)
{
   int                          Result;
   ...
   if((Buffer != NULL) && (Length >= AUDIO_ENCODER_MIN_BUFFER_SIZE) && ((Length % 4) == 0))
   {
      ...
      while((RemainingAudioFrames > 0) && (Result >= 0))
      {
         ...
         while((PlaybackContext.SBCFrameCount >= PlaybackContext.SBCFramesPerPacket) || ((BufferRemaining == 0) && (PlaybackContext.SBCFrameCount > 0)))
         {
            ...
            Result = AUDM_Send_Encoded_Audio_Data(PlaybackContext.CallbackId, ((PlaybackContext.SBCFrameCount * PlaybackContext.SBCFrameSize) + 1), &(PlaybackContext.SBCFrameBuffer[1]));
