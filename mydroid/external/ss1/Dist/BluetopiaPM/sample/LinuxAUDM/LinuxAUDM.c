...
#ifndef DISABLE_AUDIO_SINK_AUDIO_PROCESSING
...
static int AUDQueryStreamConnected(ParameterList_t *TempParam);
...
static void UserInterface(void)
{
   ...
   AddCommand("AUDQUERYSTREAMCONNECTED", AUDQueryStreamConnected);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  43)AUDQueryStreamConnected                          *\r\n");
...
static int AUDConnectAudio(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && ((TempParam->NumberofParameters == 2) || ((TempParam->NumberofParameters >= 3) && (TempParam->Params[2].intParam >= 0) && (TempParam->Params[2].intParam <= 3))) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1) && (TempParam->Params[1].strParam))
      {
         /* Parameters appear to be valid, map the specified parameters */
         /* into the API specific parameters.                           */
         StreamType = ((TempParam->Params[0].intParam == 0)?astSRC:astSNK);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDConnectAudio [Local Stream Type (0 = Source, 1 = Sink)] [BD_ADDR] [Flags (Optional - 1:Authentication, 2:Encryption)].\r\n");
...
static int AUDDisconnectAudio(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         Result = AUDM_Disconnect_Audio_Stream(((TempParam->Params[0].intParam == 0)?astSRC:astSNK));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDDisconnectAudio [Local Stream Type (0 = Source, 1 = Sink)].\r\n");
...
static int AUDPlayWAV(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
            ...
            if((FileDescriptor = open((TempParam->Params[0].strParam), (O_RDONLY))) >= 0)
            {
               if(ParseWAVHeader(FileDescriptor, &WAVInfo) == 0)
               {
                  ...
                  if(WAVInfo.Format == WAVE_FORMAT_PCM)
                  {
                     ...
                     if((Result = AUDM_Query_Audio_Stream_Connected(astSRC, &RemoteDeviceAddress)) == AUDM_AUDIO_STREAM_CONNECTED_STATE_CONNECTED)
                     {
                        if((Result = AUDM_Query_Audio_Stream_Format(astSRC, &StreamFormat)) == 0)
                        {
                           if((StreamFormat.SampleFrequency == WAVInfo.SamplesPerSecond) && (StreamFormat.NumberChannels == WAVInfo.Channels))
                           {
                              if(((Result = AUDM_Change_Audio_Stream_State(astSRC, astStreamStarted)) == 0) || (Result == BTPM_ERROR_CODE_AUDIO_STREAM_STATE_IS_ALREADY_STARTED))
                              {
               ...
               /* We're done with out copy of the file descriptor, now, so */
               /* close it.                                                */
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDPlayWAV [WAV File Name] [Restart/Resume - 0/1 (Optional)].\r\n");
...
static int AUDGetStreamStatus(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         if((Result = AUDM_Query_Audio_Stream_Connected(StreamType, &RemoteDeviceAddress)) >= 0)
         {
            printf("AUDM_Query_Audio_Stream_Connected() Success: %d.\r\n", Result);

            if(Result == AUDM_AUDIO_STREAM_CONNECTED_STATE_CONNECTED)
            {
               /* The stream is connected. Next, get the stream info.   */
               if((Result = AUDM_Query_Audio_Stream_State(StreamType, &StreamState)) == 0)
               {
                  printf("AUDM_Query_Audio_Stream_State() Success: %d.\r\n", Result);

                  if((Result = AUDM_Query_Audio_Stream_Format(StreamType, &StreamFormat)) == 0)
                  {
                     printf("AUDM_Query_Audio_Stream_Format() Success: %d.\r\n", Result);

                     /* Display info.                                   */
                     BD_ADDRToStr(RemoteDeviceAddress, Buffer);
            ...
            else
            {
               ...
               {
                  /* Stream is still being configured.                  */
                  printf("    Stream: Connecting\r\n");
               }
               else
               {
                  /* Stream not connected, just display connection      */
                  /* state.                                             */
                  printf("    Stream: Not Connected\r\n");
               }
         ...
         {
            /* Error in the API call. Inform the user and flag an error.*/
            printf("AUDM_Query_Audio_Stream_Connected() Failure: %d, %s.\r\n", Result, ERR_ConvertErrorCodeToString(Result));
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDGetStreamStatus [Local Stream Type (0 = Source, 1 = Sink)].\r\n");
   ...
   /* The following function is responsible for Querying the current    */
   /* connection status of an Audio Stream.  This function returns zero */
   /* if successful and a negative value if an error occurred.          */
static int AUDQueryStreamConnected(ParameterList_t *TempParam)
{
   int               Result;
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         if((Result = AUDM_Query_Audio_Stream_Connected(StreamType, &RemoteDeviceAddress)) >= 0)
         {
            ...
            /* Display the result to the caller.                        */
            if(Result == AUDM_AUDIO_STREAM_CONNECTED_STATE_CONNECTED)
            {
               /* Stream is connected.                                  */
               BD_ADDRToStr(RemoteDeviceAddress, Buffer);

               printf("    Stream: Connected: %s\r\n", Buffer);
            }
            else
            {
               if(Result == AUDM_AUDIO_STREAM_CONNECTED_STATE_CONNECTING)
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDQueryStreamConnected [Local Stream Type (0 = Source, 1 = Sink)].\r\n");
...
static int AUDQueryStreamState(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         Result = AUDM_Query_Audio_Stream_State(StreamType, &StreamState);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDQueryStreamState [Local Stream Type (0 = Source, 1 = Sink)].\r\n");
...
static int AUDSetStreamState(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1) && (TempParam->Params[1].intParam >= 0) && (TempParam->Params[1].intParam <= 1))
      {
         StreamType  = ((TempParam->Params[0].intParam == 0)?astSRC:astSNK);
         StreamState = ((TempParam->Params[1].intParam == 0)?astStreamStopped:astStreamStarted);

         Result = AUDM_Change_Audio_Stream_State(StreamType, StreamState);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDSetStreamState [Local Stream Type (0 = Source, 1 = Sink)] [Stream State (0 = Stop, 1 = Start)].\r\n");
...
static int AUDQueryStreamFormat(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         Result = AUDM_Query_Audio_Stream_Format(StreamType, &StreamFormat);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDQueryStreamFormat [Local Stream Type (0 = Source, 1 = Sink)].\r\n");
...
static int AUDSetStreamFormat(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      /* Make sure that all of the parameters required for this function*/
      /* appear to be at least semi-valid.                              */
      if((TempParam) && (TempParam->NumberofParameters >= 3) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         /* Note the specified parameters.                              */
         StreamType                   = ((TempParam->Params[0].intParam == 0)?astSRC:astSNK);

         StreamFormat.SampleFrequency = (unsigned long)(TempParam->Params[1].intParam);
         StreamFormat.NumberChannels  = (unsigned int)(TempParam->Params[2].intParam);
         StreamFormat.FormatFlags     = (unsigned long)0;

         Result = AUDM_Change_Audio_Stream_Format(StreamType, &StreamFormat);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDSetStreamFormat [Local Stream Type (0 = Source, 1 = Sink)] [Sample Rate] [Number Channels].\r\n");
...
static int AUDQueryStreamConfiguration(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1) && (TempParam->Params[0].intParam >= 0) && (TempParam->Params[0].intParam <= 1))
      {
         ...
         Result = AUDM_Query_Audio_Stream_Configuration(StreamType, &StreamConfiguration);
      ...
      {
         /* One or more of the necessary parameters is/are invalid.     */
         printf("Usage: AUDQueryStreamConfiguration [Local Stream Type (0 = Source, 1 = Sink)].\r\n");
...
static int AUDSendRemoteControlPassThroughCommand(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      if(AUDMRCControllerCallbackID > 0)
      {
         /* Make sure that all of the parameters required for this      */
         /* function appear to be at least semi-valid.                  */
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->Params[0].strParam) && (TempParam->Params[1].intParam >= 0))
         {
            ...
            {
               /* One or more of the necessary parameters is/are invalid.  */
               printf("Usage: AUDSendRemoteControlPassThroughCommand [BD_ADDR] [Timeout (ms)] [Passthrough Command (0 = Pause, 1 = Play, 2 = Stop, 3 = Vol. Up, 4 = Vol. Down, 5 = Specify Command)] [Specify Command Value (only used with Specify Command)].\r\n");
...
static void *PlaybackThreadMain(void *ThreadParameter)
{
   ...
   if((Params = (PlaybackThreadParams_t *)ThreadParameter) != NULL)
   {
      ...
      if((Buffer = (Byte_t *)malloc(BufferLength)) != NULL)
      {
         ...
         if(InitializeAudioEncoder(Params->CallbackID, Params->WAVInfo.SamplesPerSecond, 2) == 0)
         {
            ...
            while(((BufferRemaining > 0) || (FileRemaining > 0)) && (Result >= 0))
            {
               ...
               if(BufferRemaining == 0)
               {
                  ...
                  while((ExpectedCount > 0) && ((Result = read(Params->FileDescriptor, Buffer, ExpectedCount)) > 0))
                  {
                     ...
                     ReadCount += Result;
                     if((ReadCount % (BufferLength * 10)) == 0)
               ...
               if(Result >= 0)
               {
                  Result = SendAudioData(Buffer, BufferRemaining);
   ...
   if(EventData)
   {
      switch(EventData->EventType)
      {
         ...
         case aetAudioStreamStateChanged:
            ...
            /*          Remote Control Event Handler, in which case the */
            /*          Callback Parameter will be NULL.                */
            if((CallbackParameter) && (EventData->EventData.AudioStreamStateChangedEventData.StreamState == astStreamStopped))
            {
               /* Update the cached state and flag the notification     */
               /* event for the change.                                 */
               ((AudioStream_t *)CallbackParameter)->StreamState = EventData->EventData.AudioStreamStateChangedEventData.StreamState;
         ...
         case aetChangeAudioStreamStateStatus:
            ...
            /*          Remote Control Event Handler, in which case the */
            /*          Callback Parameter will be NULL.                */
            if((CallbackParameter) && (EventData->EventData.ChangeAudioStreamStateStatusEventData.StreamState == astStreamStopped))
            {
               /* Update the cached state and flag the notification     */
               /* event for the change.                                 */
               ((AudioStream_t *)CallbackParameter)->StreamState = EventData->EventData.ChangeAudioStreamStateStatusEventData.StreamState;
