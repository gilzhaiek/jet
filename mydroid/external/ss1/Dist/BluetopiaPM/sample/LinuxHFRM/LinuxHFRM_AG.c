...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   /* Hands-Free Commands.                                              */
   printf("*                  56)HFRRegisterEventCallback                   *\r\n");
   printf("*                  57)HFRUnRegisterEventCallback                 *\r\n");
   printf("*                  58)HFRRegisterDataCallback                    *\r\n");
   printf("*                  59)HFRUnRegisterDataCallback                  *\r\n");

   /* Application configuration commands.                               */
   printf("*                  60)SetSCOTestMode                             *\r\n");
      ...
      if(HFREventCallbackID)
      {
         ...
         if((TempParam->NumberofParameters > 0) && (TempParam->Params[0].strParam))
         {
            ...
            while((!Final) && (!Result) && (Index < MAX_NUMBER_CALL_ENTRIES))
            {
               ...
               /* verify that we have the proper number of arguments.   */
               if(Loop < 5)
               {
               ...
               if(Loop >= 5)
               {
                  ...
                  /* Note that the phone number is optional.  If it has */
                  /* not been spcified then there really isn't any need */
                  /* to process any more.                               */
                  if((Array[5]) && (Array[6]))
                  {
                     /* Get Phone Number.                               */
            ...
            /* We are done entering calls, so go ahead and dispatch the */
            /* command.                                                 */
