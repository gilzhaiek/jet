...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   if((TempParam) && (TempParam->NumberofParameters == 1))
   {
      ...
      {
         /* The first (and only) parameter should be the name of a command.*/
         /* First, make this name all upper case for searching the command */
         /* list.                                                          */
         ...
         {
            /* Reconfigure the parameter list to indicate that we want the */
            /* command-specific help printed.                              */
               ...
               if(TempParam->NumberofParameters >= 3)
               {
                  ...
                  for(Index = 2; Index < TempParam->NumberofParameters; Index++)
                  {
                     if(BTPS_StringLength(TempParam->Params[Index].strParam))
                     {
                        ...
                        {
                           /* The data string is invalid. Display an error */
                           /* and exit the parsing loop.                   */
...
int main(int argc, char* argv[])
{
   ...
   if((Result = BTPM_Initialize((unsigned long)getpid(), NULL, ServerUnRegistrationCallback, NULL)) == 0)
   {
      ...
      {
         /* Register the Device Manager callback, in case the user command */
         /* requires it.                                                   */
