...
#define AUDM_MESSAGE_FUNCTION_QUERY_AUDIO_STREAM_CONNECTED           0x00001004
...
#define AUDM_DISCONNECT_AUDIO_STREAM_RESPONSE_SIZE             (sizeof(AUDM_Disconnect_Audio_Stream_Response_t))
   ...
   /* The following structure represents the Message definition for an  */
   /* Audio (AUD) Manager Message to query the current connected state  */
   /* of an Audio Stream (Request).                                     */
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
   /*             AUDM_MESSAGE_FUNCTION_QUERY_AUDIO_STREAM_CONNECTED    */
   /*                                                                   */
   /*          Message Function ID.                                     */
typedef struct _tagAUDM_Query_Audio_Stream_Connected_Request_t
{
   BTPM_Message_Header_t MessageHeader;
   AUD_Stream_Type_t     StreamType;
} AUDM_Query_Audio_Stream_Connected_Request_t;

#define AUDM_QUERY_AUDIO_STREAM_CONNECTED_REQUEST_SIZE         (sizeof(AUDM_Query_Audio_Stream_Connected_Request_t))
   ...
   /*                                                                   */
   /*          Message Function ID.                                     */
typedef struct _tagAUDM_Query_Audio_Stream_Connected_Response_t
{
   BTPM_Message_Header_t MessageHeader;
   int                   Status;
   unsigned int          ConnectedState;
   BD_ADDR_t             RemoteDeviceAddress;
} AUDM_Query_Audio_Stream_Connected_Response_t;

#define AUDM_QUERY_AUDIO_STREAM_CONNECTED_RESPONSE_SIZE        (sizeof(AUDM_Query_Audio_Stream_Connected_Response_t))

   /* The following constants are used with the ConnectedState member of*/
   /* the AUDM_Query_Audio_Stream_Conencted_Response_t message to denote*/
   /* the various connection states of the Audio Stream.                */
#define AUDM_AUDIO_STREAM_CONNECTED_STATE_DISCONNECTED         0x00000000
#define AUDM_AUDIO_STREAM_CONNECTED_STATE_CONNECTING           0x00000001
#define AUDM_AUDIO_STREAM_CONNECTED_STATE_CONNECTED            0x00000002
