...
public class IMUCompFilterActivity extends Activity implements SensorEventListener {
	...
	private String accX_str, accY_str, accZ_str, gyroX_str, gyroY_str, gyroZ_str, magX_str, magY_str, magZ_str,prssre_str;
	...
	public void myClickHandler(View view) {
		...
		if (logData == false) {
			...
			try {
				...
				String dataline = "Start Of Logging: Acc, Giro and pressure \n\r";
				pw.write(dataline);
...
//			}
		...
		if (sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			pressure=event.values[13];
			
			accX_str = double2str(accX);
			accY_str = double2str(accY);
			accZ_str = double2str(accZ);
			prssre_str=double2str(pressure);
...
//			AccelerationBodyFrame[2]  = 0;
		...
		String dataline;
		dataline = double2str(System.currentTimeMillis()) + " , " + double2str(accX) + " , " + double2str(accY) + " , " + double2str(accZ)
	...
	private void writeDataToFile(){
		String dataline;
		dataline = double2str(System.currentTimeMillis())+",Acc," + accX_str + "," + accY_str+ "," + accZ_str + ",Giro," + gyroX_str + "," + gyroY_str + "," + gyroZ_str + "," + prssre_str;
		try{
		//pw = new BufferedWriter(new FileWriter(file));
		pw.write(dataline);
		pw.write("\n\r");
