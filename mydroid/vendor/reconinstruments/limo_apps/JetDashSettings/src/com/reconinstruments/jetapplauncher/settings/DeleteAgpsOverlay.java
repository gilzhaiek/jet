...
public class DeleteAgpsOverlay extends ReconJetDialog {
    private static final String TAG = DeleteAgpsOverlay.class.getSimpleName();

    private FragmentActivity mActivity;

    public DeleteAgpsOverlay(String title, String desc, List<Fragment> list, int layout, FragmentActivity activity) {
        ...
        getDialog().setOnKeyListener(new DialogInterface.OnKeyListener() {
            ...
            public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_DPAD_CENTER || keyCode == KeyEvent.KEYCODE_ENTER) {
                    ...
                    if (mPager.getCurrentItem() == 1) { // delete
                        LocationManager locationManager = (LocationManager)mActivity.getApplicationContext().getSystemService(mActivity.getApplicationContext().LOCATION_SERVICE);
                        locationManager.sendExtraCommand(LocationManager.GPS_PROVIDER, "delete_aiding_data", null);
                        FeedbackDialog.showDialog(mActivity, "AGPS Deleted", null,
