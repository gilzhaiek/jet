...
public abstract class TaskManagerFactory {
	...
	/**
	 * Utility method for retrieving the default argument for the task as an long.
	 * 
	 * @param taskConfig
	 *            Contains all information required to instantiate and configure
	 *            the task.
	 ...
	 * @return The value of the argument.
	 */
	protected long getDefaultLongArgument(TaskConfiguration taskConfig, long defaultValue) {
		String defaultArg;
		...
		if (defaultArg != null) {
			try {
				return Long.parseLong(defaultArg);
			} catch (NumberFormatException e) {
				throw new OsmosisRuntimeException(
					"Default argument for task " + taskConfig.getId()
					+ " must be an integer number.", e);
	...
	/**
	 * Utility method for retrieving an long argument value from a Map of
	 * task arguments.
	 * 
	 * @param taskConfig
	 *            Contains all information required to instantiate and configure
	 ...
	 * @return The value of the argument.
	 */
	protected long getLongArgument(TaskConfiguration taskConfig, String argName) {
		Map<String, String> configArgs;
		...
		if (configArgs.containsKey(argName)) {
			try {
				return Long.parseLong(configArgs.get(argName));
			} catch (NumberFormatException e) {
				throw new OsmosisRuntimeException(
					"Argument " + argName + " for task " + taskConfig.getId()
					+ " must be an long number.", e);
			}
		} else {
			throw new OsmosisRuntimeException(
				"Argument " + argName + " for task " + taskConfig.getId() + " does not exist."
	 ...
	 * @return The value of the argument.
	 */
	protected long getLongArgument(TaskConfiguration taskConfig, String argName, long defaultValue) {
		Map<String, String> configArgs;
