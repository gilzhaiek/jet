...
public interface DatasetContext extends Completable {
	...
	ReleasableIterator<EntityContainer> iterateBoundingBox2(
			double left, double right, double top, double bottom, boolean completeWays);
	
	/**
	 * Allows the  dataset to query by node id.
	 * 
	 * @return An iterator pointing to the start of the collection.
	 */
	ReleasableIterator<EntityContainer> getNodeById( long id);
	
	/**
	 * Allows the  dataset to query by way id.
	 * 
	 * @return An iterator pointing to the start of the collection.
	 */
	ReleasableIterator<EntityContainer> getWayById( long id);
	
	/**
	 * Allows the  dataset to query by relation id.
	 * 
	 * @return An iterator pointing to the start of the collection.
	 */
	ReleasableIterator<EntityContainer> getRelationById( long id);
