...
public class Activity_AsyncScanHeartRateSampler extends Activity_HeartRateDisplayBase {
     ...
     */
    protected void requestConnectToResult(final AsyncScanResultDeviceInfo
					  asyncScanResultDeviceInfo) {
        //Inform the user we are connecting
        runOnUiThread(new Runnable() {
                public void run() {
                    mTextView_Status.setText("Connecting to " +
					     asyncScanResultDeviceInfo
					     .getDeviceDisplayName());
                }
            });
        
        hrScanCtrl.requestDeviceAccess
	    (asyncScanResultDeviceInfo,
		...
		public void onResultReceived(AntPlusHeartRatePcc result,
			...
			runOnUiThread( new Runnable() {
				public void run() {
				    Toast.makeText(Activity_AsyncScanHeartRateSampler.this,
						   "Timed out attempting to connect, try again", Toast.LENGTH_LONG).show();
				    mTextView_Status.setText("Scanning for heart rate devices asynchronously...");
				}
			    });
		    }
    ...
    protected void requestAccessToPcc() {
	    ...
	    (this, 0,new IAsyncScanResultReceiver() {
		    @Override
