...
public class UpdateActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (DeviceUtils.isLimo()) {
			if( Util.LIMO_UPDATE_ZIP.isFile() && Util.LIMO_UPDATE_ZIP.exists() && Util.LIMO_COMMAND_RECOVERY.isFile() && Util.LIMO_COMMAND_RECOVERY.exists() )
			{
			...
			if (Util.JET_UPDATE_BIN_CACHE.isFile() && Util.JET_UPDATE_BIN_CACHE.exists()
					&& Util.JET_COMMAND_RECOVERY_BAK.isFile() && Util.JET_COMMAND_RECOVERY_BAK.exists()) {
				desc.setText("Software Update Available");
				item.titleAlpha = 255;
				updateList.add(item);
	...
	private OnItemClickListener updateListener = new OnItemClickListener(){
		...
		public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
			...
			if (updateList.get(position).title.equalsIgnoreCase("Update")){
				...
				if (DeviceUtils.isLimo()) {
					if(Util.LIMO_UPDATE_ZIP.isFile()&&Util.LIMO_UPDATE_ZIP.exists()&&Util.LIMO_COMMAND_RECOVERY.isFile()&&Util.LIMO_COMMAND_RECOVERY.exists()) {
						Log.d(TAG, "LIMO UPGRADING FIRMWARE : update.zip file + command file exists, copy it to cache");
						Util.doLimoFirmwareUpgrade(UpdateActivity.this);
					}
				}
				else {
					if (Util.JET_UPDATE_BIN_CACHE.isFile() && Util.JET_UPDATE_BIN_CACHE.exists()
						...
						Util.doJetFirmwareUpgrade(UpdateActivity.this);
					}
				}
			}
			else{
