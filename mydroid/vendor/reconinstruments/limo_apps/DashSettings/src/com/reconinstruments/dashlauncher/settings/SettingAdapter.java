...
//		}
		...
		if (item.checkBox || item.checkMark){
			if(item.checkBoxValue || item.checkMark){
				checkMark = (ImageView) convertView.findViewById(R.id.setting_checkmark);
				if(item.checkMark){
					checkMark.setImageResource(R.drawable.checkmark_selectable);
				}else{
					checkMark.setImageResource(R.drawable.box_on_selectable);
				}
				checkMark.setVisibility(View.VISIBLE);
			}else{
				checkMark = (ImageView) convertView.findViewById(R.id.setting_checkmark);
				checkMark.setImageResource(R.drawable.box_off_selectable);
				checkMark.setVisibility(View.VISIBLE);
			}
		}else{
			checkMark = (ImageView) convertView.findViewById(R.id.setting_checkmark);
			if(item.phoneIcon){
				checkMark.setImageResource(R.drawable.smartphone_over);
				checkMark.setVisibility(View.VISIBLE);
			}else{
