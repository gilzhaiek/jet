...
public class AdvancedActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		advancedList.add(new SettingItem("Clear Phone History"));
		...
		SettingItem demoItem = new SettingItem("Show Tutorial on Start");
		demoItem.checkBox = true;
		demoItem.checkBoxValue = Settings.System.getInt(getContentResolver(), SETTING_KEY_PLAY_INTRO_ON_START, 0) == 1;
		advancedList.add(demoItem);
	...
	private OnItemClickListener advancedListener = new OnItemClickListener(){
		...
		public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
			...
			case POS_PHONE_RESET:
				// Get all records and dismiss their notifications
				Cursor c = getContentResolver().query(PhoneLogProvider.CONTENT_URI, 
						new String[]{ PhoneLogProvider.KEY_NOTIF_ID }, null, null, null);
				while(c.moveToNext()) {
					Intent i = new Intent("RECON_DISMISS_NOTIFICATION");
			    	i.putExtra("NotificationID", c.getInt(c.getColumnIndex(PhoneLogProvider.KEY_NOTIF_ID)));
			    	AdvancedActivity.this.sendBroadcast(i);
				}
				...
				Toast.makeText(AdvancedActivity.this, "Phone History Deleted", Toast.LENGTH_LONG).show();
				break;
			}
		}
	};
