...
public class ShpPolygon extends ShpObject
{
	 ...
	 */	
	public RectXY box;			// not used as it can be erroneous, but still must be read from the buffer to access the node data
	...
	public ShpPolygon( ByteBuffer src, int size ) 
	{
		...
		if (src!=null)  {			
			...
			RectangleD bbox = new RectangleD(src.getDouble(), src.getDouble(), src.getDouble(), src.getDouble()	);
			box = new RectXY((float)bbox.topLeft.x, (float)bbox.bottomRight.y, (float)bbox.bottomRight.x, (float)bbox.topLeft.y) ;	// need to reverse top / bottom as RectangleD emulates RectF which is Y +ve down
