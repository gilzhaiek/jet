...
public class FixedFirstDataAdapter extends AbstractLocalDataAdapterWrapper
    ...
    public void updateData(int pos, LocalData data) {
        if (pos == 0) {
            mFirstData = (CameraPreviewData)data;
            if (mListener != null) {
    ...
    /**
     * By limiting the total number of data to 1 we're only showing preview of the Camera Module
     */
    @Override
    ...
    public View getView(Activity activity, int dataID) {
        if (dataID == 0) {
            return mFirstData.getView(
                    activity, mSuggestedWidth, mSuggestedHeight, null, null, false);
        }
