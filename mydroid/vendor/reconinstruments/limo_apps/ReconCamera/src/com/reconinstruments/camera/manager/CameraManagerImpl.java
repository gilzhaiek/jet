...
package com.reconinstruments.camera.manager;

import static com.reconinstruments.camera.util.CameraUtil.Assert;
...
class CameraManagerImpl implements CameraManager {
    private static final String TAG = "CAM_" + // to easily filter camera related log messages
            CameraManagerImpl.class.getSimpleName();
    ...
    private class CameraHandler extends Handler {
        ...
        public void requestTakePicture(
            ...
            // Use another thread to do the blinking so there's no delay in the
            // picture taking.
            Thread thread = new Thread() {
                @Override
                public void run() {
                    try {
                        int[] pattern = new int[] {0,250};
                        if (mLedMgr != null) mLedMgr.blinkPowerLED(HUDLedManager.BRIGHTNESS_NORMAL, pattern);
                    } catch (RuntimeException e) {
    ...
    public class AndroidCameraProxyImpl implements CameraManager.CameraProxy {
        ...
        public boolean reconnect(Handler handler, CameraOpenErrorCallback cb) {
            ...
            if (mReconnectIOException != null) {
                if (cbforward != null) {
                    cbforward.onReconnectionFailure(CameraManagerImpl.this);
                }
                return false;
            }
            return true;
