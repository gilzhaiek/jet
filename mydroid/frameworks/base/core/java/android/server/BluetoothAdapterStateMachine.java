...
final class BluetoothAdapterStateMachine extends StateMachine {
    ...
    private void broadcastState(int newState) {
        if (DBG) Log.d(TAG, "Bluetooth state " + mPublicState + " -> " + newState);
        if (mPublicState == newState) {
            return;
        }

        // Get the list of callbacks
        RemoteCallbackList<IBluetoothCallback> callbacks = mBluetoothService.getCallbacks();

        int prevState = mPublicState;
        if (callbacks !=null) {
            int n = callbacks.beginBroadcast();
            if (DBG) Log.d(TAG, "Broadcasting BT state to " + n + " receivers.");
            for (int i = 0; i <n; i++) {
                try {
                    callbacks.getBroadcastItem(i).onBluetoothStateChange(prevState, newState);
                }  catch (RemoteException e) {
        ...
        if (newState == BluetoothAdapter.STATE_ON || newState == BluetoothAdapter.STATE_OFF) {
            // Inform HUDGattService of service state changed
            Intent intent = new Intent("android.bluetooth.IBluetoothGatt");
            intent.putExtra(EXTRA_ACTION, BluetoothAdapter.ACTION_STATE_CHANGED);
