...
public class DeviceStorageMonitorService extends Binder {
    ...
    */
    ...
    private final void checkMemory(boolean checkCache) {
        ...
        if(mClearingCache) {
            ...
            // Check if the amount of free memory is less than the full threshold first, then
            // check if the amount of free memory is less than the low threshold.
            if (mFreeMem < mMemFullThreshold) {
                if (!mMemFullFlag) {
                    Slog.i(TAG, "Running critically low on memory. Sending notification");
                    sendFullNotification();
                    mMemFullFlag = true;
                } else {
                    if (localLOGV) Slog.v(TAG, "Running critically low on memory " +
                            "notification already sent. do nothing");
                }
            } else if (mFreeMem < mMemLowThreshold) {
                if (mMemFullFlag) {
    ...
    /**
     * Recon specific low memory threshold. This is set to be a percentage of
     * the total amount of memory less the full memory threshold.
     */
    private long getMemThreshold() {
        ...
        return ((mTotalMemory - getMemFullThreshold()/100L) * value);
    }
    ...
    */
    ...
    /**
     * Sends a broadcast to indicate low disk space.
     */
