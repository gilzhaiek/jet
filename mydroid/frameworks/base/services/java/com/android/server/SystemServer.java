...
class ServerThread extends Thread {
    ...
    private static final int RECON_DEFAULT_MEMORY_THRESHOLD = 1024*1024*60; // 60MB
    ...
    public void run() {
        ...
        PcbtemperatureService pcb_temperature = null;
        ...
        BluetoothManagerService bluetoothManager = null;
        ...
        try {
            ...
            Slog.i(TAG, "PCB temperature Service");
            pcb_temperature = new PcbtemperatureService(context);
            //ServiceManager.addService("pcb_temperature", pcb_temperature);
            ...
            if (SystemProperties.get("ro.kernel.qemu").equals("1")) {
                ...
                bluetoothManager = new BluetoothManagerService(context);
                ServiceManager.addService(BluetoothAdapter.BLUETOOTH_MANAGER_SERVICE, bluetoothManager);
        ...
        try {
            if (!"1".equals(SystemProperties.get("recon_boot.done"))) {
                Log.d(TAG, "recon_boot.sh still running. Show map unzipping");
                ActivityManagerNative.getDefault().showBootMessage("Maps", true);
            } else {
                Log.d(TAG, "recon_boot.sh completed");
        ...
        if (factoryTest != SystemServer.FACTORY_TEST_LOW_LEVEL) {
           ...
           try {
            ...
            // Set the full threshold before starting the DeviceStorageMonitorService
            Settings.Secure.putInt(mContentResolver,
                                   Settings.Secure.SYS_STORAGE_FULL_THRESHOLD_BYTES,
                                   RECON_DEFAULT_MEMORY_THRESHOLD);
