...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    private static final int MESSAGE_INIT_AFTER_REGISTRATION = 400;
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED.equals(action)) {
                ...
                */
                Log.d(TAG, "Ignoring user switched");
    ...
    private final boolean isBluetoothPersistedStateOn() {
        return Settings.System.getInt(mContentResolver,
                Settings.System.BLUETOOTH_ON, 0) != BLUETOOTH_OFF;
    }
    ...
    private final boolean isBluetoothPersistedStateOnBluetooth() {
        return Settings.System.getInt(mContentResolver,
                Settings.System.BLUETOOTH_ON, 0) == BLUETOOTH_ON_BLUETOOTH;
    }
    ...
    public void initAfterRegistration() {
        if (DBG) {
            Log.d(TAG, "initAfterRegistration()");
        }

        Message msg = mHandler.obtainMessage(MESSAGE_INIT_AFTER_REGISTRATION);
        mHandler.sendMessage(msg);
    }
    ...
    private class BluetoothServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            } else if (className.getClassName().equals("com.reconinstruments.bluetooth.gatt.HUDGattService")) {
            ...
            switch (msg.what) {
                case MESSAGE_GET_NAME_AND_ADDRESS: {
                    ...
                    synchronized(mConnection) {
                        ...
                        if ((mBluetooth == null) && (!mBinding)) {
                            ...
                            // Get the IBluetooth interface
                            IBinder b = ServiceManager.getService(BluetoothAdapter.BLUETOOTH_SERVICE);
                            if (b != null) {
                                mBinding = true;
                                Message message = mHandler.obtainMessage(MESSAGE_BLUETOOTH_SERVICE_CONNECTED);
                                message.arg1 = SERVICE_IBLUETOOTH;
                                message.obj = b;
                                mHandler.sendMessage(message);
                            } else {
                                Log.e(TAG, "Failed to bind to: " + IBluetooth.class.getName());
                            }
                        }
                        else {
                ...
                case MESSAGE_BLUETOOTH_SERVICE_CONNECTED:
                {
                    ...
                    synchronized(mConnection) {
                        ...
                        try {
                            if (DBG) Log.d(TAG, "Registering IBluetoothCallback to IBluetooth");
    ...
    private boolean checkIfCallerIsForegroundUser() {
        /*
        int foregroundUser;
        int callingUser = UserId.getCallingUserId();
        int callingUid = Binder.getCallingUid();
        long callingIdentity = Binder.clearCallingIdentity();
        int callingAppId = UserId.getAppId(callingUid);
        boolean valid = false;
        try {
        ...
        if (prevState != newState) {
            ...
            if (newState == BluetoothAdapter.STATE_ON || newState == BluetoothAdapter.STATE_OFF) {
                ...
                if (isUp) {
                    ...
                    Log.d(TAG, "Checking to see if BLE feature enabled on the system");
                    if (mContext.getPackageManager().hasSystemFeature(
                                                     PackageManager.FEATURE_BLUETOOTH_LE)) {
                        Intent i = new Intent(IBluetoothGatt.class.getName());
                        Log.d(TAG, "Binding to IBluetoothGatt");
