...
public class PowerManagerService extends IPowerManager.Stub
    ...
    // Setting autobrightness steps
    static final int AUTOBRIGHTNESS_ANIM_STEPS = ANIM_STEPS;
    ...
    // Setting autodimness steps
    static final int AUTODIMNESS_ANIM_STEPS = ANIM_STEPS;
    ...
    private UnsynchronizedWakeLock mDisableSuspendPartialLock;
    ...
    private static final int HUDSCREEN_STATE_FORCED_ON = 5;
    private static final int HUDSCREEN_STATE_FORCED_STAY_ON = 6;
    ...
    */
    ...
    void initInThread() {
        ...
        mDisableSuspendPartialLock = new UnsynchronizedWakeLock(
                                PowerManager.PARTIAL_WAKE_LOCK, "NeverSuspend Partial", true);
    ...
    private void updateWakeLockLocked() {
        ...
        /* retreive hw.disable_suspend value and update corresponding lock */
        boolean bNeverSuspend = Boolean.parseBoolean(SystemProperties.get("hw.disable_suspend"));
        if (bNeverSuspend && !mDisableSuspendPartialLock.isHeld())
            mDisableSuspendPartialLock.acquire();
        else if (!bNeverSuspend && mDisableSuspendPartialLock.isHeld())
            mDisableSuspendPartialLock.release();
    ...
    private int getHUDScreenState() {
        int screenState = -1;

        // Create class loader to load HUDScreenManager class
        ...
        try {
            if (mHUDScreenMgr == null) {
                ...
                // Retrieve the HUDScreenManager.getInstance() static method
            ...
            Slog.e(TAG, "Failed to find HUD class");
        } catch (IllegalAccessException e) {
            Slog.e(TAG, "Illegal access to HUDScreenManager");
        } catch (Exception e) {
            Slog.e(TAG, "Exception executing HUDScreenManager. " + e.getCause());
        }

        if (DEBUG_SCREEN_ON) Slog.d(TAG, "Retrieved HUD screenState: " + screenState);
    ...
    class ScreenBrightnessAnimator extends HandlerThread {
        ...
        private int hudScreenState;
        private boolean doSetBrightness;
                ...
                public void handleMessage(Message msg) {
                    ...
                    if (msg.what == ANIMATE_LIGHTS) {
                        ...
                        if ((mask & SCREEN_BRIGHT_BIT) != 0) {
                            ...
                            if (doSetBrightness) mLcdLight.setBrightness(value, brightnessMode);
                        }
        ...
        private void animateInternal(int mask, boolean turningOff, int delay) {
            synchronized (this) {
                if (currentValue != endValue) {
                    ...
                    // Finished animating brightness to 0. Now turn off the screen.
        ...
        public void animateTo(int target, int mask, int animationDuration) {
            if (isModelJet) {
                // Retreieve the HUDScreenManager screen state. Don't need to turn on the screen here
                // if HUDScreenManager already forced turned on the screen
                hudScreenState = getHUDScreenState();
            }
            if (target != 0 &&
                (hudScreenState == HUDSCREEN_STATE_FORCED_ON || hudScreenState == HUDSCREEN_STATE_FORCED_STAY_ON)) {
                if (mDebugLightAnimation) {
                    Slog.v(TAG, "Updating mPowerState: " + dumpPowerState(mPowerState));
                }
                doSetBrightness = false;
                mPowerState |= SCREEN_ON_BIT;
                updateNativePowerStateLocked();
            } else {
                doSetBrightness = true;
            }
    ...
    public void reboot(String reason)
    {
        ...
        //JET-624-------------
        //we want the device to reboot in order to charge fully and then shutdown
        if (reason.equals("BegForShutdown") && mIsCharging){
            Slog.d(TAG, "Charge full then shutdown");
            reason = "ChargeShutdown";
        }
    ...
    public String getLastShutdownReason() {
        return ShutdownThread.getLastShutdownReason();
    }
    ...
    public void setBrightness(int id, int brightness) {
        Slog.d(TAG, "setBrightness - id:" + id + " brightness:" + brightness);
        if (id < LightsService.LIGHT_ID_COUNT) mLightsService.getLight(id).setBrightness(brightness);
        else Slog.d(TAG, "Invalid id");
    }

    public int getBrightness(int id) {
        Slog.d(TAG, "getBrightness - id:" + id);
        if (id < LightsService.LIGHT_ID_COUNT) return mLightsService.getLight(id).getBrightness();
        else {
            Slog.d(TAG, "Invalid id");
            return -1;
        }
    }

    public void setFlashing(int id, int color, int onMs, int offMs) {
        Slog.d(TAG, "setFlashing - id:" + id + " color:" + color +
                    " onMs:" + onMs + " offMs:" + offMs);
        if (id < LightsService.LIGHT_ID_COUNT)
            mLightsService.getLight(id).setFlashing(color, LightsService.LIGHT_FLASH_TIMED, onMs, offMs);
        else Slog.d(TAG, "Invalid id");
    }
