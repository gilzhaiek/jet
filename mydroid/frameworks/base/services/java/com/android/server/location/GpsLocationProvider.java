...
public class GpsLocationProvider implements LocationProviderInterface {
    ...
    private static final int GPS_STATUS_FATAL_ERROR = 5;
...
// IMPORTANT - the GPS_DELETE_* symbols here must match constants in gps.h
    ...
    // turn off GPS fix icon if we haven't received a fix in 5 seconds
    private static final long RECENT_FIX_TIMEOUT = 5 * 1000;
    ...
    private DisableThread mDisableThread;
    private CountDownLatch mDisableLatch;
    private boolean mFatalDisabling;
    // Counter to keep track of how many times we've restarted the GPS HAL due to
    // fatal errors.
    private static int mFatalRestartCounter = 0;
    private static final int MAX_FATAL_RESTARTS = 100;
    ...
    private void handleDisable() {
        ...
        native_cleanup(mFatalDisabling);
    ...
    private void startNavigating(boolean singleShot) {
        if (!mStarted) {
            ...
            // Start the AGPS service whenever we start navigating
            Intent intent = new Intent("com.reconinstruments.agps.AssistedGpsService");
    ...
    private void stopNavigating() {
        ...
        if (mStarted) {
            // Stop the AGPS service whenever we stop navigating
            Intent intent = new Intent("com.reconinstruments.agps.AssistedGpsService");
     ...
     */
    private void reportLocation(int flags, double latitude, double longitude, double altitude, double msl_altitude,
            float speed, float bearing, float accuracy, long timestamp) {
        if (VERBOSE) Log.v(TAG, "reportLocation lat: " + latitude + " long: " + longitude +
                " timestamp: " + timestamp + " altitude: " + altitude + " msl_altitude: " + msl_altitude);
        ...
        synchronized (mLocation) {
            ...
            if ((flags & LOCATION_HAS_ALTITUDE) == LOCATION_HAS_ALTITUDE) {
                ...
                mLocationExtras.putDouble("msl_altitude", msl_altitude);
            } else {
    ...
    private class DisableThread extends Thread {
        private boolean mNav;

        public DisableThread(boolean navigating) {
            mNav = navigating;
            Log.d(TAG, "Init disable thread. Was navigating: " + mNav);
        }
        @Override
        public void run() {
            if (DEBUG) Log.d(TAG, "Disable GPS HAL");
            disable();
            // Wait for disabling HAL to be completed before we continue on.
            while (true) {
                try {
                    mDisableLatch.await();
                    break;
            ...
            if (mFatalRestartCounter >= MAX_FATAL_RESTARTS) {
                // We've exceeded the maximum number of fatal restarts.
                // We don't want to restart again.
                Log.e(TAG, "Received fatal error. Exceeded number of fatal restarts: " + mFatalRestartCounter);
            	return;
            }

            if (DEBUG) Log.d(TAG, "Enable GPS HAL");
            enable();
            if (mNav) {
                if (DEBUG) Log.d(TAG, "Starting location tracking");
                enableLocationTracking(true);
            }
            mFatalDisabling = false;
            mFatalRestartCounter++;
        }
    }
        ...
        synchronized(mListeners) {
            ...
            switch (status) {
                ...
                case GPS_STATUS_ENGINE_OFF:
                    ...
                    mNavigating = false;
                    break;
                case GPS_STATUS_FATAL_ERROR:
                    if (!mFatalDisabling) {
                        mDisableLatch = new CountDownLatch(1);
                        mDisableThread = new DisableThread(mNavigating);
                        mFatalDisabling = true;
                        mEngineOn = false;
                        mNavigating = false;
                        mDisableThread.start();

                        // Broadcast intent to notify AGPS that a restart is needed
                        Intent intent = new Intent("com.reconinstruments.restartAGPS");
                        mContext.sendBroadcast(intent);
                    } 
                    break;
            }
    ...
    private native void native_cleanup(boolean stop_server);
