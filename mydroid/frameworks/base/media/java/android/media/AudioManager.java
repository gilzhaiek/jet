...
public class AudioManager {
    ...
    /** @hide */
    public static final int AUDIO_CUT_OFF_FREQ = 500000; // Cut off frequency in milli Hz
    ...
    */
    ...
    public IRingtonePlayer getRingtonePlayer() {
     ...
     /**
     * Set a high pass filter on an audio session
     * @param audioSessionId The audio session ID to be affected
     * @param cutOffFreq The frequency in milli Hz to set the high pass filter
     * {@hide}
     */
    public static void setHighPassFilter(int audioSessionId, int cutOffFreq) {
        Equalizer equalizer = new Equalizer(EFFECT_PRIORITY, audioSessionId);
        ...
        if (DEBUG) Log.d(TAG, "Audio session [" + audioSessionId + "] cut off at: " + cutOffFreq + " hasControl(): " + equalizer.hasControl());
        if (DEBUG) Log.d(TAG, minLevel + " - " + maxLevel);

        for (int i = 0; i < numBands; i++)
        {
            int[] freqRange = equalizer.getBandFreqRange((short)i);
            if (DEBUG) Log.d(TAG, "BAND["+i+"] " + formatBandLabel(freqRange) + " = " + equalizer.getBandLevel((short)i));

            if (freqRange[1] < cutOffFreq && freqRange[1] >= 1000) {
                equalizer.setBandLevel((short)i, (short)minLevel);
            } else {
                equalizer.setBandLevel((short)i, (short)(minLevel+(maxLevel-minLevel)/2));
            }
    ...
    private static String formatBandLabel (int[] band)
    {
