...
public class BleClientService extends Service {
    ...
    private final BluetoothGattCallback mGattCallbacks = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    Log.d(TAG, "onConnectionStateChange connected... status: " + status);
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.d(TAG, "onServiceDiscovered success... status: " + status);
                notifyServicesDiscovered();
            }
            else
            {
                Log.d(TAG, "onServiceDiscovered failed... status: " + status);

                BluetoothDevice device = gatt.getDevice();
                int state = gatt.getConnectionState(device);

                Log.d(TAG, "onServiceDiscovered failed... device UUID: " + device.getAddress());
                Log.d(TAG, "onServiceDiscovered failed... device state: " + state);
            }
            }
