...
#define PROXMUX_IOCTL_SET_ENABLE  _IOW  (PROXMUX_IOCTL_BASE, 1, int) 

// Sensor Poll Rate: Compatibility with Android API - puts sensor to default (Kernel) queue   
#define PROXMUX_IOCTL_SET_DELAY   _IOW  (PROXMUX_IOCTL_BASE, 2, int)  

// Board Configuration: Mask of all registered sensors   
#define PROXMUX_IOCTL_GET_CONFIG  _IOWR  (PROXMUX_IOCTL_BASE, 3, int)  
...
#define PROXMUX_IOCTL_START_QUEUE _IOW (PROXMUX_IOCTL_BASE, 5, int)    

// Stop Sensor Reporting Queue
#define PROXMUX_IOCTL_STOP_QUEUE  _IOW  (PROXMUX_IOCTL_BASE, 6, int)

// Set  Sensor Reporting Mode
#define PROXMUX_IOCTL_SET_MODE    _IOW  (PROXMUX_IOCTL_BASE, 7, int)

// Set  Sensor Fastest allowed reporting rate
#define PROXMUX_IOCTL_SET_MIN_DELAY _IOW (PROXMUX_IOCTL_BASE, 8, int)

//TODO: Expose Queue configuration query?

/*Define Warning return code*/
#define NO_DELAY_SETTING 1000 //Some sensors such as IRQ based sensor, no need to set delay time
#define NO_DELAY_SETTING_FOR_SENSOR_QUEUE 1001 // If Sensors are arleady in the queue, they will use the same possible minimum delay time
#endif
