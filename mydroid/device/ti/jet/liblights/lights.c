...
char const*const PWBUTTON_LED_FILE
        = "/sys/class/leds/pwbutton_light/brightness";
char const*const PWBUTTON_DELAY_ON_FILE
        = "/sys/class/leds/pwbutton_light/delay_on";
char const*const PWBUTTON_DELAY_OFF_FILE
        = "/sys/class/leds/pwbutton_light/delay_off";
...
read_int(char const* path)
{
    ...
    if (fd >= 0) {
        char buffer[20];
        ssize_t size = read(fd, buffer, 20);
        close(fd);
        int val = atoi(buffer);
        if (size == -1) {
            ALOGE("read_int failed to read from fd!");
            return -errno;
        }
        return val;
    } else {
        ...
        struct light_state_t const* state)
{
    ...
    //The sequence is important here, first on, then brightness,then off
    write_int(PWBUTTON_DELAY_ON_FILE, onMS);
    err = write_int(PWBUTTON_LED_FILE, blue);
    write_int(PWBUTTON_DELAY_OFF_FILE, offMS);
...
static int
get_light_notification()
{
    return read_int(PWBUTTON_LED_FILE);
}
        ...
        struct hw_device_t** device)
{
    ...
    if (0 == strcmp(LIGHT_ID_BACKLIGHT, name)) {
        ...
        get_light = get_light_notification;
    }
