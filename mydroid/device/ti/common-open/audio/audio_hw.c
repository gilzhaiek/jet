...
#define MIXER_ABE_GAIN_20DB                 140
...
#define SHORT_PERIOD_SIZE (ABE_BASE_FRAME_COUNT * SHORT_PERIOD_MULTIPLIER * LONG_PERIOD_MULTIPLIER)
#define LONG_PERIOD_SIZE SHORT_PERIOD_SIZE
#else
...
#define CAPTURE_MAIN_MIC_VOLUME               20
#define CAPTURE_SUB_MIC_VOLUME                20
#define CAPTURE_HEADSET_MIC_VOLUME            20

#define VOICE_RECOGNITION_MAIN_MIC_VOLUME     20
#define VOICE_RECOGNITION_SUB_MIC_VOLUME      25
#define VOICE_RECOGNITION_HEADSET_MIC_VOLUME  25

#define CAMCORDER_MAIN_MIC_VOLUME             20
#define CAMCORDER_SUB_MIC_VOLUME              20
#define CAMCORDER_HEADSET_MIC_VOLUME          20

#define VOIP_MAIN_MIC_VOLUME                  20
#define VOIP_SUB_MIC_VOLUME                   20
#define VOIP_HEADSET_MIC_VOLUME               20

#define VOICE_CALL_MAIN_MIC_VOLUME            20
#define VOICE_CALL_SUB_MIC_VOLUME             20
#define VOICE_CALL_HEADSET_MIC_VOLUME         20
...
#define VOICE_CALL_REC_UL_VOLUME              20
#define VOICE_CALL_REC_DL_VOLUME              20
...
#define PRODUCT_DEVICE_OMAP4_JET   "jet"
/* fm */
...
struct route_setting defaults_omap4_jet[] = {
    /* general */
    {
        .ctl_name = MIXER_DL1_EQUALIZER,
        .strval = MIXER_FLAT_RESPONSE,
...
#endif
struct route_setting jet_output_on[] ={
   {
        .ctl_name = MIXER_SIDETONE_MIXER_PLAYBACK,
        .intval = 1,
   },
...
struct route_setting mm_ul2_dmic0_jet[] = {//one channel so far
    {
        .ctl_name = MIXER_MUX_UL10,//MIXER_MUX_UL00 goes to mm_ul2
        .strval = MIXER_DMIC0L,
    },
#ifdef STEREO_MIC
    {
        .ctl_name = MIXER_MUX_UL11,//MIXER_MUX_UL00 goes to mm_ul2
        .strval = MIXER_DMIC0R,
    },
#endif
    {
...
struct route_setting vx_ul_dmic0_jet[] = {
    {
        .ctl_name = MIXER_MUX_VX0,
        .strval = MIXER_DMIC0L,
    },
...
static int get_boardtype(struct omap_audio_device *adev)
{
    ...
    else if(!strcmp(board, PRODUCT_DEVICE_OMAP4_JET)) {
        adev->board_type = OMAP4_JET;
        adev->sidetone_capture = 1;
    }
    else
        return -EINVAL;
                              ...
                              int headset_mic_on, int sub_mic_on)
{
    ...
    LOGFUNC("%s(%p, %d, %d, %d, %d, %p)", __FUNCTION__, adev, main_mic_on,
                                headset_mic_on, sub_mic_on,adev->mode,adev->active_input);

    if (adev->mode == AUDIO_MODE_IN_CALL) {
#ifdef JET_AUDIO
        volume = DB_TO_ABE_GAIN(main_mic_on ? VOICE_CALL_MAIN_MIC_VOLUME : 0);
#else
...
#ifdef JET_AUDIO
        volume = DB_TO_ABE_GAIN(main_mic_on ? CAPTURE_MAIN_MIC_VOLUME : 0);
#else
...
#ifdef JET_AUDIO
        volume = DB_TO_ABE_GAIN(main_mic_on ? CAMCORDER_MAIN_MIC_VOLUME : 0);
#else
...
#ifdef JET_AUDIO
    //TODO: Set volume for SDT DL Volume, DL1 Media Playback Volume and DL1 Tones Playback Volume
#else
    ...
    struct omap_stream_out *out)
{
    ...
    if (out) {
        LOGFUNC("%s(%p) Out Devices=%x", __FUNCTION__, out, out->device);
    }
    /* Mute VX_UL to avoid pop noises in the tx path
     * during call before switch changes.
     */
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        if(adev->board_type != OMAP4_JET) { //We will return the audio call in back to mobile phone
...
#ifdef JET_AUDIO
    set_route_by_array(adev->mixer, jet_output_on, 1);
    //mixer_ctl_set_value(adev->mixer_ctls.dl1_mono, 0, 1);
#else
    /* select output stage */
        ...
        if (bt_on)
            ...
            if (adev->board_type == OMAP4_JET) {
                if(headset_on || headphone_on || earpiece_on)
                    set_route_by_array(adev->mixer, vx_ul_dmic0_jet, 1);
                else
                    set_route_by_array(adev->mixer, vx_ul_dmic0_jet, 0);
            } else {
                if (headset_on || headphone_on || earpiece_on) {
                    set_route_by_array(
                            adev->mixer,
...
static void select_input_device(struct omap_audio_device *adev)
{
    ...
    LOGFUNC("%s(%p)", __FUNCTION__, adev);
#ifndef JET_AUDIO//Jet has only main mic
    if (!bt_on) {
...
#ifdef JET_AUDIO
    set_route_by_array(adev->mixer, mm_ul2_dmic0_jet, 1);
    main_mic_on=1;
#else
    else if (fm_rx_on) {
...
static unsigned int test_mixer_ctls(struct mixer_ctls mixer_ctls)
{
    unsigned int mode=0;
    mode |= (!mixer_ctls.mm_dl1);
    mode |= ((!mixer_ctls.vx_dl1)<<1);
    mode |= ((!mixer_ctls.dl1_mono)<<2);
    mode |= ((!mixer_ctls.tones_dl1)<<3);
    mode |= ((!mixer_ctls.dl1_headset)<<4);
    mode |= ((!mixer_ctls.dl1_bt)<<5);
    mode |= ((!mixer_ctls.earpiece_enable)<<6);//not support for jet
    mode |= ((!mixer_ctls.left_capture)<<7);//not support for jet
    mode |= ((!mixer_ctls.right_capture)<<8);//not support for jet
    mode |= ((!mixer_ctls.amic_ul_volume)<<9);
    mode |= ((!mixer_ctls.sidetone_capture)<<10);
    mode |= ((!mixer_ctls.headset_volume)<<11);//not support for jet
    mode |= ((!mixer_ctls.speaker_volume)<<12);//not support for jet
    mode |= ((!mixer_ctls.dmic1_ul_volume)<<13);
    mode |= ((!mixer_ctls.dl1_eq)<<14);
    mode |= ((!mixer_ctls.voice_ul_volume)<<15);
    mode |= ((!mixer_ctls.mm2_dl1_capture)<<16);
    LOGFUNC("%s(0x%x)\n", __FUNCTION__, mode);
    return mode;
}
                     ...
                     hw_device_t** device)
{
...
#ifdef LOG_NDEBUG
    test_mixer_ctls(adev->mixer_ctls);
#endif
#if 0
    ...
    }else if(OMAP4_JET == adev->board_type) {
        set_route_by_array(adev->mixer, defaults_omap4_jet, 1);
    }
    else {
        set_route_by_array(adev->mixer, defaults, 1);
    }
    if(OMAP4_JET != adev->board_type){
        if (adev->dl2_support) {
