...
// control power clock for camera
static void powerControl(const char* s)
{
    int clockFd=open("/sys/devices/platform/cam_mt9m114/camera_power", O_RDWR);
    int err;
    
    if(clockFd >= 0){
      err=write(clockFd, s , sizeof(s));
      if(err<0){
          CAMHAL_LOGEB("Error During Camera Clock Enable -0x%x", err);
      }
      close(clockFd);
    }
}
