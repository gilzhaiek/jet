...
/* Related to JET-712, the omap4_hwc_reset_screen function will power cycle the kopin.
 * The Kopin driver itself will take care of the first image instead of uboot, so comment it out*/
//#define BOOTLOADER_IMAGE_REMOVE
...
static int omap4_hwc_device_open(const hw_module_t* module, const char* name, hw_device_t** device)
{
    ...
    if (strncmp("jet", value, PROPERTY_VALUE_MAX) == 0) {
        ALOGI("Revert to legacy HWC API for fake vsync");
        hwc_dev->base.common.version = HWC_DEVICE_API_VERSION_0_2;
    }
