...
#ifdef BT_ANT_SWITCH
#define BT_MODE_CONF_FILE		"/data/misc/recon/BT.conf"
#define BT_MODE		"/sys/devices/platform/kim/bt_mode"
#define BLE_MODE '0';
#define ANT_MODE '1';
/*
 * comment this one becuase the SS1 stack is able to send patch to switch between BLE and ANT
 */
//#define WILINK_FIRMWARE_CHANGE

/**
 * Function to decide which firmware to load: BLE or ANT
 */
void check_bt_mode(void)
{
	int fd;
	int err;
#ifdef WILINK_FIRMWARE_CHANGE
	char mode[2]={0};
	char buffer[10];
#endif
	fd = open(BT_MODE_CONF_FILE,  O_RDONLY);
	if(fd<0){
		UIM_ERR("fail to open BT_MODE_FILE");
		fd= open(BT_MODE_CONF_FILE, O_WRONLY|O_CREAT);
		if(fd<0){
			UIM_ERR("fail to create %s (%d)",BT_MODE_CONF_FILE, errno);
			return;
		}
		err = write(fd, "mode=ANT", 9);

		if(err<0)
			UIM_ERR("fail to write %s (%d)",BT_MODE_CONF_FILE, errno);
		close(fd);
		err=chmod(BT_MODE_CONF_FILE, 0666);
		if(err<0){
			UIM_ERR("change mode failed for %s (%d)", BT_MODE_CONF_FILE, errno);
		}
		return;
	}
#ifdef WILINK_FIRMWARE_CHANGE
	...
	if(err<0){
		UIM_ERR("fail to read BT_MODE_FILE");
		return;
	}
	//TODO: make complex version of the string check
	if(strncmp(buffer, "mode=ANT",8)==0){
		mode[0]=ANT_MODE;
	}
	else
		mode[0]=BLE_MODE;
	UIM_DBG("%s,mode=%c",__func__,mode[0]);
	close(fd);

	fd = open(BT_MODE,  O_WRONLY);
	if(fd<0){
		UIM_ERR("fail to open BT_MODE");
		return;
	}

	err= write(fd,mode, sizeof(mode));
	if(err<0){
		UIM_ERR("fail to write BT_MODE");
	}
#endif
	close(fd);
}
...
RE_POLL:
	...
	while (!exiting) {
		p.revents = 0;
		err = poll(&p, 1, -1);//If the downloading fine, the uim service will stop here and keep polling
