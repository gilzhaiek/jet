...
MAKE_PATCH_CMD="./build/tools/releasetools/ota_from_target_files -v -n -k build/target/product/security/testkey -u out/target/product/jet/boot/u-boot.bin -i ${ORIGIN_IMAGE} ${NEW_IMAGE} ${UPDATE_FILE}"
