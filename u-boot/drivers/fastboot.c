...
/* This is the GOOGLE USB vendor id */
//#define DEVICE_VENDOR_ID  0x18D1
/* This is the GOOGLE USB vendor id */
#define DEVICE_VENDOR_ID  0x2523
/* This is just made up.. */
#define DEVICE_PRODUCT_ID 0xD100
#define DEVICE_BCD        0x0100;

/* This is used to get the serial number */
#if defined(CONFIG_4430SDP) || defined(CONFIG_4430PANDA) || defined(CONFIG_4430JET)
...
#if defined(CONFIG_4430JET)
void set_main_led(int on) {
    unsigned char data;
...
#define PWRBTN_RELEASED		((data & STS_PWRON)==1)
#define PWRBTN_PRESSED		((data & STS_PWRON)==0)
#define SAMPLE_PWRBTN		i2c_read(TWL6030_CHIP_PM, PMC_MASTER_STS_HW_CONDITIONS, 1, &data, 1)
#define SELECT_BTN_PRESSED	(omap_get_gpio_datain(GPIO_BUTTON_INPUT)==0)
#define DBG_PRINT_PWRBTN	printf("[%d] pwrbutton reg=0x%.2x\n",__LINE__,data);
#define S2_SUN_DELAY_CNT        8
#define PRESS_MIN_CNT		3
#define S3_5_7_SUN_BLINK_MAX_CNT	16
#define S4_6_SUN_PRESS_MAX_CNT	20
#define S7_SUN_PRESS_MAX_CNT	20
#define S3_DELAY_CNT		16
#define S4_QUICK_BLINK_CNT	10
#define S4_DELAY_CNT		8
#define S8_TIMEOUT		40
#define UDELAY_VFAST		2000
#define UDELAY_FAST		10000
#define UDELAY_SLOW		500000
...
#define SR_SUN	10
#define SC_SUN	11
	// S1: Checking if PWRBTN is pressed
	// S2: Blink while waiting for the release of the power button
	// S3: Wait for the long press of the selet button (continue if power/timeout)
	// S4: Blink and wait for the release of the Select button
	// S5: Wait for the long press of the power button (continue if select/timeout)
	// S6: Blink and wait for the release of the Power button
	// S7: Wait for the long press of the selet button (continue if power/timeout)
	// S8: Blink very fast and wait for the release of the select button (continue if timeout)
	// SR: Starting Hard Recovery
	// SC: Continue to normal boot

	curr_state = S1_SUN;
	while((curr_state != SR_SUN) && (curr_state != SC_SUN)) {
		switch (curr_state) {
		case S1_SUN :
			printf("S1 Sun: Checking if PWRBTN is pressed\n");
			SAMPLE_PWRBTN;
			if (PWRBTN_RELEASED) {
				curr_state = SC_SUN;
			} else {
			...
			break;
		case S2_SUN :
			printf("S2 Sun: Blink while waiting for the release of the power button\n");
			do {
				set_main_led(0);
				udelay(UDELAY_FAST);
				SAMPLE_PWRBTN;
				...
				udelay(UDELAY_FAST);
				SAMPLE_PWRBTN;
			} while(PWRBTN_PRESSED);

			MV(CP(UART3_RX_IRRX),(IEN|M3)); // Change "Select" button to GPIO mode
		...
		case S5_SUN:
		case S7_SUN:
			if(curr_state == S5_SUN) {
				printf("S5 Sun: Wait for the long press of the power button (continue if select/timeout)\n");
			} else { // S3 or S7
				printf("S%d Sun: Wait for the long press of the selet button (continue if power/timeout)\n", curr_state);
			}

			for(i = 0; i < S3_5_7_SUN_BLINK_MAX_CNT; i++) {
				for(j = 0; (j < 2) && (curr_state <= S7_SUN); j++) {
					udelay(UDELAY_FAST);

					// Check the select button first
					if(SELECT_BTN_PRESSED) {
						if(curr_state == S5_SUN) {
							printf("S5.%d Sun: Exit State - select button pressed instead of power button\n", j);
							curr_state = SC_SUN;
							break; // for j
						} else { // S3 or S7
							if(press_cnt++ > PRESS_MIN_CNT) {
								curr_state++;
								break; // for j
							}
						}
					} else if(curr_state != S5_SUN) { // Reset the press count counter if select button is not pressed while in state 3 or 7
						press_cnt = 0;
					}

					// Check the power button next
					SAMPLE_PWRBTN; // Exit (S3/S7) if Power button is pressed or continue if S5
					if(PWRBTN_PRESSED) {
						if(curr_state == S5_SUN) {
							if(press_cnt++ > PRESS_MIN_CNT) {
								curr_state++;
							...
							printf("S%d.%d Sun: Exit State - power button pressed instead of select button\n", curr_state, j);
							curr_state = SC_SUN;
							break; // for j
						}
					} else if(curr_state == S5_SUN) { // Reset the press count counter if power button is not pressed while in state 5
						press_cnt = 0;
					}
					j++;
				} // for j
				if((curr_state != S3_SUN) && (curr_state != S5_SUN) && (curr_state != S7_SUN)) {
					break; // for i
				}
			} // for i

			if(i == S3_5_7_SUN_BLINK_MAX_CNT) { // Max cnt reached (timeout)
				printf("S%d Sun: Exit State - max cnt reached (timeout)\n", curr_state);
				curr_state = SC_SUN;
			}
			break;
		case S4_SUN:
		case S6_SUN:
			if(curr_state == S4_SUN) {
				printf("S4 Sun: Blink and wait for the release of the Select button\n");
			} else { // S6_SUN
				printf("S6 Sun: Blink and wait for the release of the Power button\n");
			}
			for(i = 0; i < S4_6_SUN_PRESS_MAX_CNT; i++) {
				for(j = 0; j < 2; j++) {
					set_main_led(j); // OFF and ON
					udelay(UDELAY_FAST);
					SAMPLE_PWRBTN;
					if(	((curr_state == S4_SUN) && (!SELECT_BTN_PRESSED)) ||
						((curr_state == S6_SUN) && (PWRBTN_RELEASED))) {
						curr_state++;
						break;
					}
				}
				if(j != 2) break; // Button Release
			}
			if(i == S4_6_SUN_PRESS_MAX_CNT) { // Max cnt reached (timeout)
				printf("S%d Sun: Exit State - max cnt reached\n", curr_state);
				curr_state = SC_SUN;
			}
			set_main_led(1);
			break;
		case S8_SUN:
			printf("S8 Sun: Blink very fast and wait for the release of the select button (timeout will continue)\n");
			...
			do {
				...
				if(i >= S8_TIMEOUT) {
					printf("S8 Sun: Exit State - max cnt reached\n");
					curr_state = SC_SUN;
				} else {
					printf("S8 Sun: Hard Reset\n");
					curr_state = SR_SUN;
				}
			} while(SELECT_BTN_PRESSED && (++i < S8_TIMEOUT));

			while(++i < 20) { // Finish the blinking to be consistent
				for(j = 0; j < 2; j++) {
					udelay(UDELAY_VFAST);
					set_main_led(j); // OFF and ON
				}
			...
			break;
		default:
			printf("ERROR SUN: Unknown state = %d\n",curr_state);
			curr_state = SC_SUN;
		}
	}

//	Can't change back to UART as the keystrokes stop from booting
//	MV(CP(UART3_RX_IRRX),(IEN|M0)); // Change "Select" back to UART3_RX mode

	if(curr_state == SR_SUN) {
		goto hard_reset_sr;
	} else {
		goto hard_reset_sc;
	}
...
#else
	// Enter Hard Reset States only if USB is not connected
	// S1: Checking if PWRBTN is pressed
	// S2: PWRBTN is pressed, Waiting for PWRBTN Release
	// S3: PWRBTN Released, checking for PWRBTN Press for the next ~2 seconds
	// S4: Blink Real Fast and wait for release of PWRBTN and than blink normal and press within 4 seconds
	// S5: Blink Real Fast and wait for release of PWRBTN
	// SR: Starting Hard Recovery

	// Dont go into hard reset if USB is connected
...
hard_reset_s1_snow:
	printf("S1 Snow: Checking if PWRBTN is pressed\n");
...
hard_reset_s2_snow:
	printf("S2 Snow: PWRBTN is pressed, Waiting for PWRBTN Release\n");

	while (PWRBTN_PRESSED) {
		SAMPLE_PWRBTN;
	}
hard_reset_s3_snow:
	printf("S3 Snow: PWRBTN Released, checking for PWRBTN Press for the next ~2 seconds\n");

	i = S3_DELAY_CNT;
	while (PWRBTN_RELEASED && (i)) {
		SAMPLE_PWRBTN;
		i--;
	}
	if(i == 0) {
		printf("S3 Snow: Recovery Canceled: Timeout for re-press reached\n");
		goto hard_reset_sc;
	}

hard_reset_s4_snow:
	printf("S4 Snow: Blink Real Fast and wait for release of PWRBTN and than blink normal and press for the next 4 seconds\n");

	while(s4_cnt < 2) {
		while (PWRBTN_PRESSED) { // Waiting for a Release
			set_main_led(0);
			udelay(UDELAY_SLOW);
			SAMPLE_PWRBTN;
			set_main_led(1);
		...
		i = S4_QUICK_BLINK_CNT;
		while (i--) { // Quick Blink to show a state transition
			set_main_led(0);
			udelay(UDELAY_FAST);
			set_main_led(1);
			udelay(UDELAY_FAST);
		...
		i = S4_DELAY_CNT;
		while (i) { // Wait for a press - timeout is S4_DELAY_CNT seconds
			SAMPLE_PWRBTN;
			if(PWRBTN_PRESSED) {
				break;
			}
		...
		if(i == 0) { // Timeout
			printf("S4 Snow: Recovery Canceled: Timeout Reached\n");
			goto hard_reset_sc;
		}
		s4_cnt++;
	}
...
hard_reset_s5_snow:
	printf("S5 Snow: Blink Real Fast and wait for release of PWRBTN\n");
	while (PWRBTN_PRESSED) { // Waiting for a Release
		set_main_led(0);
		SAMPLE_PWRBTN;
		set_main_led(1);
...
hard_reset_sr:
	printf("SR: Starting Hard Recovery\n");
	strcpy(cmd[3], "hard");
	cmd_length=4;
	goto start_recovery;

hard_reset_sc:
	printf("SC: Continue normal boot\n");

#endif // CONFIG_4430JET
...
start_recovery:
			...
			do_booti(NULL, 0, cmd_length, cmd);
...
#elif defined(CONFIG_4430JET)

	device_strings[DEVICE_STRING_MANUFACTURER_INDEX]  = "Recon Instruments";
#ifdef CONFIG_SUN
	device_strings[DEVICE_STRING_PRODUCT_INDEX]       =  "Jet_Fastboot";
#else
	device_strings[DEVICE_STRING_PRODUCT_INDEX]       = "Snow2_Fastboot";
#endif
...
#error "Need a product name for fastboot"
	...
	static char device_rid[MAX_USB_SERIAL_NUM];
	unsigned int rid_val;
	...
	printf("Recon Device ID: %d\n",  __raw_readl(0x4A326100));
	sprintf(device_serial, "%08X%08X", val[1], val[0]);
	sprintf(device_rid, "%d", __raw_readl(0x4A326100));
