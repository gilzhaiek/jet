...
int do_booti (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	rid_val = __raw_readl(0x4A326100);
	if ((rid_val >= 260000000) && (rid_val < 300000000)) {
		serial_len = sprintf(serial_str, " androidboot.serialno=%d", rid_val);
	} else {
	...
	if (extracmd_str) {
		...
		if (argc > 3){
			if (!strcmp(argv[3],"hard"))
			{
				...
				strncpy(boot_str, extracmd_str, extracmd_len);
				/*Add one more string*/
				strcat(boot_str, FACTORY_RESET);
				extracmd_len+=strlen(FACTORY_RESET);
				extracmd_str=boot_str;
				printf("[%s:%u] boot_str=%s extracmd_str=%s\n",__FUNCTION__,__LINE__,boot_str,extracmd_str);
			}
		}
#endif
